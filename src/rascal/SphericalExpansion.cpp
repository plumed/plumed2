  ...
  ActionWithValue(ao)
{
  ...
  addValue( shape ); setNotPeriodic(); getPntrToComponent(0)->buildDataStore(); getPntrToComponent(0)->reshapeMatrixStore( shape[1] );
  checkRead();
}

template <class T>
unsigned RascalSpherical<T>::getNumberOfDerivatives() {
...
void RascalSpherical<T>::calculate() {
  ...
  Value* valout=getPntrToComponent(0); std::vector<unsigned> shape( valout->getShape() );
...
void RascalSpherical<T>::apply() {
  ...
  if( !getPntrToComponent(0)->forcesWereAdded() ) return ;
  ...
  Value* outval=getPntrToComponent(0); Tensor vir; vir.zero();
  double lunit = 1.0; if( !plumed.usingNaturalUnits() ) lunit = 10*plumed.getUnits().getLength();
