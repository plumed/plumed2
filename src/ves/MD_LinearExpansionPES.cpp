...
int MD_LinearExpansionPES::main( FILE* in, FILE* out, PLMD::Communicator& pc) {
    ...
    if(pc.Get_rank() == 0) {
        ...
        for(unsigned int i=1; i<plumed_inputfiles.size(); i++) {
            fprintf(out,",%s",plumed_inputfiles[i].c_str());
        }
        fprintf(out,"\n");
        fprintf(out,"kBoltzmann taken as 1, use NATURAL_UNITS in the plumed input\n");
        if(diff_input_coeffs) {
            fprintf(out,"using different coefficients for each replica\n");
        }
    }
    ...
    if(potential_expansion_pntr) {
        delete potential_expansion_pntr;
    }
    if(coeffs_pntr) {
        delete coeffs_pntr;
    }
