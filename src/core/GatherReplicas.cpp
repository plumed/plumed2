...
unsigned GatherReplicas::getNumberOfDerivatives() const {
...
void GatherReplicas::calculate() {
  ...
  unsigned nvals = myarg->getNumberOfValues( getLabel() ), nder = myarg->getNumberOfDerivatives();
  ...
  for(unsigned k=0; k<nreplicas; k++) {
      Value* myout = getPntrToOutput(k); unsigned sstart=k*nvals*(1+nder);
      for(unsigned i=0;i<nvals;++i) {
          myout->set( i, datap[sstart+i*(1+nder)] );
          if( myarg->getRank()==0 ) { for(unsigned j=0;j<nder;++j) myout->setDerivative( j, dval[i*(1+nder)+1+j] ); }
          else if( myarg->hasDerivatives() ) { for(unsigned j=0;j<nder;++j) myout->setGridDerivative( i, j, dval[i*(1+nder)+1+j] ); }
      }
  }
}
