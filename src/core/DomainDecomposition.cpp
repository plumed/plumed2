...
DomainDecomposition::DomainDecomposition(const ActionOptions&ao):
  ...
  for(unsigned i=1;; ++i) {
    ...
    if( constant=="True") {
      plumed.readInputLine( valname + ": PUT FROM_DOMAINS CONSTANT SHAPE=" + str_natoms + " UNIT=" + unit + " PERIODIC=" + period + " ROLE=" + role, true );
    } else if( constant=="False") {
      plumed.readInputLine( valname + ": PUT FROM_DOMAINS SHAPE=" + str_natoms + " UNIT=" + unit + " PERIODIC=" + period + " ROLE=" + role, true );
    } else {
