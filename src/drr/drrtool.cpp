...
void drrtool::mergewindows(const vector<string> &filename) {
    ...
    std::transform(std::begin(tmp_name), std::end(tmp_name), std::begin(tmp_name), [&](string s) {
        return s.substr(0, s.find(suffix));
    });
    string mergename = std::accumulate(std::begin(tmp_name), std::end(tmp_name), string(""), [](string a, string b) {
        return a + b + "+";
    });
