...
SketchMapRead::SketchMapRead( const ActionOptions& ao ):
  ...
  for(unsigned i=0; i<nlow; ++i) {
    std::size_t dot=propnames[i].find_first_of( getLabel() + "." );
    std::string substr=propnames[i].c_str();
    if( dot!=std::string::npos ) {
      substr.erase(dot,getLabel().length()+1);
    }
    log.printf(",%s", propnames[i].c_str() );
    addComponent( substr );
    componentIsNotPeriodic( substr );
...
// call fclose when fp goes out of scope
  ...
  while (do_read) {
    ...
    for(unsigned i=0; i<inpdb.getArgumentNames().size(); ++i) {
      ...
      if( !inpdb.getArgumentValue( aname, val ) ) {
        error("failed to find argument named " + aname);
      }
  ...
  for(unsigned i=0; i<myframes.size(); ++i) {
    weights[i] /= wnorm;
    myframes[i]->getAtomRequests( atoms, skipchecks );
