  ...
  ActionShortcut(ao)
{
  ...
  double cutoff=0, h; std::string kfunc,fname; 
  ...
  for(unsigned k=0;; ++k) {
      ...
      double mu_w, mu_i, mu_j, mu_k; std::string hstr, smu_w, smu_i, smu_j, smu_k, sigmastr; Tools::convert( h, hstr );
      ifile.scanField("mu_w",mu_w); Tools::convert( mu_w, smu_w ); 
      ifile.scanField("mu_i",mu_i); Tools::convert( mu_i, smu_i ); 
      ifile.scanField("mu_j",mu_j); Tools::convert( mu_j, smu_j ); 
      ifile.scanField("mu_k",mu_k); Tools::convert( mu_k, smu_k ); 
      double sigma; ifile.scanField("sigma",sigma); Tools::convert( sigma, sigmastr ); 
      ifile.scanField(); if( k==0 ) kfunc = hstr; else kfunc += "+" + hstr; 
      kfunc += "*exp(" + sigmastr + "*(w*" + smu_w + "+i*" + smu_i + "+j*" + smu_j + "+k*" + smu_k + ")^2)"; 
  }
  ...
  readInputLine( getShortcutLabel() + "_cmat: CONTACT_MATRIX  " + grpinfo + " SWITCH={" + switchstr + "}");
  // 
  if( specA.length()==0 ) {
  ...
  // 
  readInputLine( getShortcutLabel() + "_kfunc: CUSTOM ARG=" + getShortcutLabel() + "_quatprod.* VAR=w,i,j,k PERIODIC=NO FUNC=" + kfunc );
  ...
  //
  readInputLine( getShortcutLabel() + ": MATRIX_VECTOR_PRODUCT ARG=" + getShortcutLabel() + "_kdmat," + getShortcutLabel() + "_ones");
}
