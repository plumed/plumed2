...
void ActionWithAveraging::update() {
    ...
    if ( weights.size()>0 && !my_analysis_object ) {
        double sum=0;
        for(unsigned i=0; i<weights.size(); ++i) sum+=weights[i]->get();
        lweight=sum;
        cweight = exp( sum );
    } else {
        lweight=0;
    ...
    if( !my_analysis_object && normalization==ndata ) normt = 1;
...
void ActionWithAveraging::performTask( const unsigned& task_index, const unsigned& current, MultiValue& myvals ) const {
    if( my_analysis_object ) {
        ...
        if( normalization==f ) myvals.setValue( 1+getNumberOfArguments(), 1.0 );
        else myvals.setValue( 1+getNumberOfArguments(), 1.0 / cweight );
