  ...
  ActionWithValue(ao)
{
  ...
  bool scalars=false; std::string fname, vname; parse("FILE",fname);
  ...
  if( fname.length()>0 ) {
    ...
    parseVector("VALUES",vals); parseFlag("SCALARS",scalars);
    if( vals.size()==0 ) {
      parseVector("VALUE",vals);
      if( vals.size()!=1 ) error("VALUE keyword should take a single scalar");
    } else if( vals.size()==1 ) scalars=false;

    log.printf("  read in %d values :", vals.size() );
    if( !nolog ) { for(unsigned i=0; i<vals.size(); ++i) log.printf(" %f", vals[i] ); }
    log.printf("\n");
    if( !scalars && shape.size()==0 && vals.size()>1 ) { shape.resize(1); shape[0] = vals.size(); }
  }
  if( !scalars ) {
    // Now set the value
    ...
    for(unsigned i=0; i<vals.size(); i++) {
      ...
      addComponent("v-"+num); componentIsNotPeriodic("v-"+num);
      Value* comp=getPntrToComponent("v-"+num);
      comp->setConstant(); comp->set(vals[i]);
    }
  }
}
