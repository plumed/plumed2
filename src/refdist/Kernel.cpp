  ...
  ActionShortcut(ao)
{
  ...
  if( argnames.size()==0 ) error("no arguments were specified");
  ...
  if( fname.length()>0 ) {
    ...
    for(unsigned line=0; line<number; ++line) {
      for(unsigned i=0; i<argnames.size(); ++i) {
        ...
        if( line==number-1 ) readInputLine( getShortcutLabel() + "_" + fixArgumentDot(argnames[i]) + "_ref: CONSTANT VALUES=" + val );
      }
      if( ifile.FieldExist("sigma_" + argnames[0]) ) {
        ...
        if( line==number-1 ) readInputLine( getShortcutLabel() + "_var: CONSTANT VALUES=" + varstr );
      } else {
        ...
        if( line==number-1 ) {
          if( !usemahalanobis ) readInputLine( getShortcutLabel() + "_var: CONSTANT VALUES=" + varstr );
          else readInputLine( getShortcutLabel() + "_cov: CONSTANT NCOLS=" + nvals + " NROWS=" + nvals + " VALUES=" + varstr );
        }
      }
    ...
    for(unsigned i=0; i<argnames.size(); ++i) readInputLine( getShortcutLabel() + "_" + fixArgumentDot(argnames[i]) + "_ref: CONSTANT VALUES=" + center[i] );
