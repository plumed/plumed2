...
double RMSDVector::calculateRMSD( const unsigned& current, std::vector<Vector>& pos, std::vector<Vector>& der, std::vector<Vector>& direction ) const {
  ...
  if( displacement && type=="SIMPLE" ) {
    ...
    double r = myrmsd[current].calc_PCAelements( pos, der, rot, DRotDPos, direction, centeredpos, centeredreference, squared ); std::vector<Vector> ref( myrmsd[current].getReference() );
    if( !doNotCalculateDerivatives() && myval->forcesWereAdded() ) {
      ...
      for(unsigned n=0; n<natoms; n++) {
        for(unsigned k=0; k<3; ++k) direction[n][k] = centeredreference[n][k];
      }
    } else {
...
void RMSDVector::calculate() {
  ...
  if( getPntrToComponent(0)->getRank()==0 ) {
    ...
    if( getNumberOfComponents()==2 ) {
      ...
      for(unsigned i=0; i<natoms; i++) {
        for(unsigned j=0; j<3; ++j ) mydisp->set( j*natoms+i, direction[i][j] );
      }
    }
...
void RMSDVector::performTask( const unsigned& current, MultiValue& myvals ) const {
  ...
  std::vector<Vector>& der( allder[0] ); std::vector<Vector>& direction( allder[1] );
  if( pos.size()!=natoms ) { pos.resize( natoms ); der.resize( natoms ); direction.resize( natoms ); }
  for(unsigned i=0; i<pos.size(); ++i) {
    for(unsigned j=0; j<3; ++j) pos[i][j] = getPntrToArgument(0)->get( j*natoms + i );
  }
