...
private:
 ...
 vector<Vector> gmm_m_;
 vector<double> gmm_w_;
 vector< Matrix <double> > gmm_cov_;
 void get_radii(vector<AtomNumber> &atoms);
 void read_GMM_datafile(string gmm_file);
public:
...
void Overlap::registerKeywords( Keywords& keys ){
  ...
  keys.addOutputComponent("comp", "COMPONENTS","overlap with individual GMM components");
}
...

{
  ...
  // read GMM data file
  read_GMM_datafile(gmm_file);
  log.printf("  number of GMM components : %d\n", gmm_m_.size());
  
  for(unsigned i=0;i<gmm_m_.size();++i) {
      string num; Tools::convert(i,num);
      addComponentWithDerivatives("comp"+num); componentIsNotPeriodic("comp"+num);  
  }
...
void Overlap::get_radii(vector<AtomNumber> &atoms)
{
  ...
  if( moldat.size()==1 ){
    ...
    for(unsigned i=0;i<atoms.size();++i){
      ...
      if(radii_map.find(type_s) != radii_map.end()){
        radii_.push_back(radii_map[type_s]);
      } else {
...
// read GMM data file in IMP or PLUMED format:
// |Id|Weight|mean|covariance matrix|
void Overlap::read_GMM_datafile(string gmm_file)
{
 int idcomp;
 ...
 if(ifile->FileExist(gmm_file)){
    ...
    while(ifile->scanField("Id",idcomp)){
     ...
     gmm_w_.push_back(w);
     gmm_m_.push_back(Vector(m0,m1,m2));
...
void Overlap::calculate(){
  ...
  for(unsigned i=0;i<gmm_m_.size();++i) {
      string num; Tools::convert(i,num);
      Value* value=getPntrToComponent("comp"+num);
      value->set(0);
      for(unsigned j=0;j<getNumberOfAtoms();j++){
         setAtomsDerivatives (value,j,Vector(0,0,0));
      }
  }
