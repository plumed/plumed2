...
//lambda stuff
  ...
  keys.add("optional","STEPS_LAMBDA","manually set the number of intermediate temperatures");
//deltaFs file
...
//printing some info
  ...
  if(steps_lambda_!=0)
    log.printf(" +++ WARNING +++ STEPS_LAMBDA is used, thus OBSERVATION_STEP is set to 1\n"
               "                 Reference field and initial deltaFs can be set whith a fake RESTART\n");
  if(walkers_mpi)
...
//restart if needed
  if(getRestart())
  {
    ...
    if(ifile.FileExist(deltaFsFileName_))
    {
      ...
      plumed_massert(steps_lambda_==deltaFsName.size()-pre_f-post_f,"RESTART - something wrong with '"+deltaFsFileName_+"' file: steps not matching size");
...
void OPESmultiLambda::calculate()
{
  ...
  {
    const long double add_i=std::exp(static_cast<long double>(beta_*((lambda_-lambda_p_[i])*field+deltaF_[i])));
    sum+=add_i;
  ...
  current_bias_=-1./beta_*std::log(sum/steps_lambda_);
...
//calculate work
  if(calc_work_)
  {
    ...
    for(unsigned i=rank_; i<steps_lambda_; i+=NumParallel_)
      old_sum+=std::exp(static_cast<long double>(beta_*((lambda_-lambda_p_[i])*field+old_deltaF_[i])));
    if(NumParallel_>1)
      comm.Sum(old_sum);
...
//update averages. this assumes that calculate() always runs before update(), thus uses current_bias_
  ...
  if(NumWalkers_==1)
  {
    ...
    for(unsigned i=0; i<steps_lambda_; i++)
      deltaF_[i]+=-increment-1./beta_*std::log1p(std::exp(static_cast<long double>(beta_*((lambda_-lambda_p_[i])*field+current_bias_+shiftC_+deltaF_[i])))/(counter_-1.));
  ...
  else
  {
    ...
    for(unsigned w=0; w<NumWalkers_; w++)
    {
      ...
      for(unsigned i=0; i<steps_lambda_; i++)
        deltaF_[i]+=-increment_w-1./beta_*std::log1p(std::exp(static_cast<long double>(beta_*((lambda_-lambda_p_[i])*all_field[w]+all_bias[w]+shiftC_+deltaF_[i])))/(counter_-1.));
...
unsigned OPESmultiLambda::estimate_steps(const double left_side,const double right_side,const std::vector<double>& obs,const std::string msg) const
{
  ...
  auto get_neff_HWHM=[](const double side,const std::vector<double>& obs) //HWHM = half width at half maximum. neff is in general not simmetric
  {
