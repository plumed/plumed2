...
//+PLUMEDOC OPES_BIAS OPES_EXPANDED_TEST
/*
...
Such ensembles can be sampled via methods like replica exchange, or this OPES_EXPANDED_TEST bias action.
...
Contrary to \ref OPES_METAD, OPES_EXPANDED_TEST does not use kernel density estimation.
...
opes: OPES_EXPANDED_TEST ARG=ecv.* PACE=500
PRINT FILE=COLVAR STRIDE=500 ARG=ene,opes.bias
\endplumedfile

You can easily combine multiple ECVs.
The OPES_EXPANDED_TEST bias will create a multidimensional target grid to sample all the combinations.
...
opes: OPES_EXPANDED_TEST ARG=ecv1.*,ecv2.* PACE=500 OBSERVATION_STEPS=1
...
opes: OPES_EXPANDED_TEST ARG=mtp.*,umb.* PACE=500 WALKERS_MPI PRINT_STRIDE=1000
...
class OPESexpanded_test : public bias::Bias {
...
public:
  explicit OPESexpanded_test(const ActionOptions&);
  ...
  inline void updateNeffStuff();
  inline double getExpansion(const unsigned) const;
};

PLUMED_REGISTER_ACTION(OPESexpanded_test,"OPES_EXPANDED_TEST")

void OPESexpanded_test::registerKeywords(Keywords& keys)
{
...
//output components
  ...
  keys.addOutputComponent("activeDeltaFs","NEFF_THRESHOLD","number of active DeltaFs, with big enough associated neff");
}

OPESexpanded_test::OPESexpanded_test(const ActionOptions&ao)
  : PLUMED_BIAS_INIT(ao)
  , isFirstStep_(true)
...
//add output components
  ...
  {
    addComponent("activeDeltaFs");
    componentIsNotPeriodic("activeDeltaFs");
  }
...
void OPESexpanded_test::calculate()
{
...
//set bias and forces
  current_bias_=-kbt_*(diffMax+std::log(sum/(deltaF_size_-neff_skipped_)));
...
void OPESexpanded_test::update()
{
  ...
  plumed_massert(afterCalculate_,"OPESexpanded_test::update() must be called after OPESexpanded_test::calculate() to work properly");
  afterCalculate_=false;
...
void OPESexpanded_test::init_pntrToECVsClass()
{
...
void OPESexpanded_test::init_linkECVs()
{
...
void OPESexpanded_test::init_from_obs() //This could probably be faster and/or require less memory...
{
...
//link ECVs and initialize index_k_, mapping each deltaF to a single ECVs set
  ...
  if(neff_threshold_>0)
  {
    sum_weights_.resize(deltaF_.size());
    sum_weights2_.resize(deltaF_.size());
    neff_ok_.resize(deltaF_.size(),false);
    neff_skipped_=deltaF_size_;
  }
...
//for the first point, t=0, the ECVs are calculated by initECVs_observ, setting also any initial guess
  ...
  for(unsigned i=0; i<deltaF_.size(); i++)
  {
    ...
    if(neff_threshold_>0)
    {
      const long double w_i=std::exp(static_cast<long double>(-getExpansion(i)));
      sum_weights_[i]+=w_i;
      sum_weights2_[i]+=w_i*w_i;
    }
  }
...
inline void OPESexpanded_test::updateDeltaF(double bias)
{
  ...
  #pragma omp parallel num_threads(NumOMP_)
  {
    ...
    for(unsigned i=0; i<deltaF_.size(); i++)
    {
      ...
      if(neff_threshold_>0 && !neff_ok_[i])
      {
        const long double w_i=std::exp(static_cast<long double>(-getExpansion(i)+bias/kbt_));
        sum_weights_[i]+=w_i;
        sum_weights2_[i]+=w_i*w_i;
      }
    }
...
inline double OPESexpanded_test::getExpansion(unsigned i) const
{
  double expansion=0;
  for(unsigned j=0; j<ncv_; j++)
