...
//+PLUMEDOC OPES_BIAS OPES_METAD_TEST
/*
...
This \ref OPES_METAD_TEST action samples target distributions defined via their marginal \f$p^{\text{tg}}(\mathbf{s})\f$ over some collective variables (CVs), \f$\mathbf{s}=\mathbf{s}(\mathbf{x})\f$.
By default \ref OPES_METAD_TEST targets the well-tempered distribution, \f$p^{\text{WT}}(\mathbf{s})\propto [P(\mathbf{s})]^{1/\gamma}\f$, where \f$\gamma\f$ is known as BIASFACTOR.
Similarly to \ref METAD, \ref OPES_METAD_TEST optimizes the bias on-the-fly, with a given PACE.
...
As an intuitive picture, rather than gradually filling the metastable basins, \ref OPES_METAD_TEST quickly tries to get a coarse idea of the full free energy surface (FES), and then slowly refines its details.
...
Notice that \ref OPES_METAD_TEST is more sensitive to degenerate CVs than \ref METAD.
If the employed CVs map different metastable basins onto the same CV-space region, then \ref OPES_METAD_TEST will remain stuck rather than completely reshaping the bias.
This can be useful to diagnose problems with your collective variable.
If it is not possible to improve the set of CVs and remove this degeneracy, then you might instead consider to use \ref OPES_METAD_TEST_EXPLORE or \ref METAD.
...
On the contrary, if your CVs are not degenerate but only suboptimal, you should converge faster by using \ref OPES_METAD_TEST instead of \ref METAD \cite Invernizzi2020rethinking.
...
If another bias potential is used besides \ref OPES_METAD_TEST, it is possible to take into account also for it during the internal reweighting for estimating \f$P(\mathbf{s})\f$.
...
Another possible usage of EXTRA_BIAS_ARG is to make sure that \ref OPES_METAD_TEST does not push against another fixed bias added to restrain the CVs range.
...
opes: OPES_METAD_TEST ARG=cv PACE=200 BARRIER=40
...
template <class mode>
class OPESmetadTest : public bias::Bias {
...
//neighbour list stuff
  ...
  //EXCLUDED_INTERVAL
  std::vector<double> excluded_interval_;
...
public:
  explicit OPESmetadTest(const ActionOptions&);
...
typedef OPESmetadTest<convergence> OPESmetadTest_c;
PLUMED_REGISTER_ACTION(OPESmetadTest_c,"OPES_METAD_TEST")

//OPES_METAD_TEST_EXPLORE is very similar from the point of view of the code,
//but conceptually it is better to make it a separate BIAS action

//+PLUMEDOC OPES_BIAS OPES_METAD_TEST_EXPLORE
/*
...
This \ref OPES_METAD_TEST_EXPLORE action samples the well-tempered target distribution, that is defined via its marginal \f$p^{\text{WT}}(\mathbf{s})\propto [P(\mathbf{s})]^{1/\gamma}\f$ over some collective variables (CVs), \f$\mathbf{s}=\mathbf{s}(\mathbf{x})\f$.
While \ref OPES_METAD_TEST does so by estimating the unbiased distribution \f$P(\mathbf{s})\f$, \ref OPES_METAD_TEST_EXPLORE instead estimates on-the-fly the target \f$p^{\text{WT}}(\mathbf{s})\f$ and uses it to define the bias.
The bias at step \f$n\f$ is
\f[
...
Intuitively, while \ref OPES_METAD_TEST aims at quickly converging the reweighted free energy, \ref OPES_METAD_TEST_EXPLORE aims at quickly sampling the target well-tempered distriution.
...
Compared to \ref OPES_METAD_TEST, \ref OPES_METAD_TEST_EXPLORE is more similar to \ref METAD, because it allows the bias to vary significantly, thus enhancing exploration.
This goes at the expenses of a typically slower convergence of the reweight estimate.
\ref OPES_METAD_TEST_EXPLORE can be useful e.g.~for simulating a new system with an unknown BARRIER, or for quickly testing the effectiveness of a new CV that might be degenerate.

Similarly to \ref OPES_METAD_TEST, also \ref OPES_METAD_TEST_EXPLORE uses a kernel density estimation with the same on-the-fly compression algorithm.
...
All the options of \ref OPES_METAD_TEST are also available in \ref OPES_METAD_TEST_EXPLORE, except for those that modify the target distribution, since only a well-tempered target is allowed in this case.
...
\plumedfile
cv: DISTANCE ATOMS=1,2
opes: OPES_METAD_TEST_EXPLORE ARG=cv PACE=500 BARRIER=40
...
typedef OPESmetadTest<exploration> OPESmetadTest_e;
// For some reason, this is not seen correctly by cppcheck
// cppcheck-suppress unknownMacro
PLUMED_REGISTER_ACTION(OPESmetadTest_e,"OPES_METAD_TEST_EXPLORE")

template <class mode>
void OPESmetadTest<mode>::registerKeywords(Keywords& keys)
{
...
//miscellaneous
  keys.add("optional","EXCLUDED_INTERVAL","an interval on the first cv where bias should not be put");
  if(!mode::explore)
...
template <class mode>
OPESmetadTest<mode>::OPESmetadTest(const ActionOptions& ao)
  : PLUMED_BIAS_INIT(ao)
  , isFirstStep_(true)
  , counter_(1)
...
//parallelization stuff
  ...
  parseVector("EXCLUDED_INTERVAL",excluded_interval_);
  if(excluded_interval_.size()>0)
  {
    plumed_massert(excluded_interval_.size()%2==0,"need two points for defining an interval");
    plumed_massert(excluded_interval_.size()<=2*ncv_,"two points for each CV, not more");
    for(unsigned i=0; i<ncv_; i++)
    {
      plumed_massert(excluded_interval_[2*i]<excluded_interval_[2*i+1],"is this an interval?");
      log.printf(" -- EXCLUDED_INTERVAL: no bias will be placed from %g to %g\n",excluded_interval_[2*i],excluded_interval_[2*i+1]);
    }
  }
...
template <class mode>
void OPESmetadTest<mode>::calculate()
{
//get cv
...
template <class mode>
void OPESmetadTest<mode>::update()
{
...
//update variance if adaptive sigma
  ...
  //check EXCLUDED_INTERVAL
  bool not_excluded=true;
  if(getStep()%stride_==0 && excluded_interval_.size()>0)
  {
    unsigned is_excluded=0;
    for(unsigned i=0; i<excluded_interval_.size()/2; i++)
    {
      const double cv_i=getArgument(i);
      if(cv_i>excluded_interval_[2*i] && cv_i<excluded_interval_[2*i+1])
        is_excluded++;
    }
    not_excluded=!(is_excluded==excluded_interval_.size()/2);
  }
...
template <class mode>
double OPESmetadTest<mode>::getProbAndDerivatives(const std::vector<double>& cv,std::vector<double>& der_prob)
{
  double prob=0.0;
  if(!nlist_)
  {
...
template <class mode>
void OPESmetadTest<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma)
{
  bool no_match=true;
  if(threshold2_!=0)
  {
...
template <class mode>
void OPESmetadTest<mode>::addKernel(const double height,const std::vector<double>& center,const std::vector<double>& sigma,const double logweight)
{
  addKernel(height,center,sigma);
//write to file
...
template <class mode>
unsigned OPESmetadTest<mode>::getMergeableKernel(const std::vector<double>& giver_center,const unsigned giver_k)
...
template <class mode>
void OPESmetadTest<mode>::updateNlist(const std::vector<double>& new_center)
{
...
template <class mode>
void OPESmetadTest<mode>::dumpStateToFile()
{
...
template <class mode>
inline double OPESmetadTest<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x) const
...
template <class mode>
inline double OPESmetadTest<mode>::evaluateKernel(const kernel& G,const std::vector<double>& x, std::vector<double>& acc_der, std::vector<double>& dist)
{ //NB: cannot be a method of kernel class, because uses external variables (for cutoff)
  double norm2=0;
  for(unsigned i=0; i<ncv_; i++)
  {
...
template <class mode>
inline void OPESmetadTest<mode>::mergeKernels(kernel& k1,const kernel& k2)
{
