...
class ColvarCoordinationNumber : public ColvarWithModifiers {
...
ColvarWithModifiers(ao)
{
  ...
  setNeighbourListStyle("skipBigOnly");
  registerKeyword(2,"GROUP","calculate the coordination numbers of all the atoms in the first group.  It is assumed that each group defines an atom type.  Hence, if there is one group present one is calculating the number of atoms of the same type that are within the coordination sphere.  If there are two groups present then one is calculating the number of atoms of from GROUP2 that are within the coordination sphere");
...
// calculator
double ColvarCoordinationNumber::compute( const std::vector<unsigned>& indexes, std::vector<Vector>& derivatives, Tensor& virial ) {
 assert( indexes.size()==derivatives.size() ); 

 double ncoord=0, dfunc; Vector distance;
 for(unsigned n=1; n<indexes.size();++n){
    distance=getSeparation( indexes[0], indexes[n] );
    dfunc=0.; ncoord += switchingFunction.calculate(distance.modulo(), dfunc);
    derivatives[0] = derivatives[0] + (-dfunc)*distance;
    derivatives[n] = derivatives[n] + dfunc*distance;
