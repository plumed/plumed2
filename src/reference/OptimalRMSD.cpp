...
double OptimalRMSD::calc( const std::vector<Vector>& pos, ReferenceValuePack& myder, const bool& squared ) const {
  ...
  if( myder.calcUsingPCAOption() ) {
    ...
    for(unsigned i=0; i<nat; ++i) {
      myder.getAtomsDisplacementVector()[i] -= getReferencePosition(i);
      myder.getAtomsDisplacementVector()[i] *= getDisplace()[i];
    }
  } else if( fast ) {
    if( getAlign()==getDisplace() ) {
      d=myrmsd.optimalAlignment<false,true>(getAlign(),getDisplace(),pos,getReferencePositions(),myder.getAtomVector(),squared);
    } else {
      d=myrmsd.optimalAlignment<false,false>(getAlign(),getDisplace(),pos,getReferencePositions(),myder.getAtomVector(),squared);
    }
  } else {
    if( getAlign()==getDisplace() ) {
      d=myrmsd.optimalAlignment<true,true>(getAlign(),getDisplace(),pos,getReferencePositions(),myder.getAtomVector(),squared);
    } else {
      d=myrmsd.optimalAlignment<true,false>(getAlign(),getDisplace(),pos,getReferencePositions(),myder.getAtomVector(),squared);
    }
  }
  myder.clear();
  for(unsigned i=0; i<pos.size(); ++i) {
    myder.setAtomDerivatives( i, myder.getAtomVector()[i] );
  }
...
void OptimalRMSD::extractAtomicDisplacement( const std::vector<Vector>& pos, std::vector<Vector>& direction ) const {
  ...
  unsigned nat = pos.size();
  for(unsigned i=0; i<nat; ++i) {
    direction[i] = getDisplace()[i]*( direction[i] - getReferencePosition(i) );
  }
}
...
double OptimalRMSD::projectAtomicDisplacementOnVector( const bool& normalized, const std::vector<Vector>& vecs, ReferenceValuePack& mypack ) const {
  ...
  for(unsigned a=0; a<3; a++) {
    for(unsigned b=0; b<3; b++) {
      ...
      for(unsigned n=0; n<getNumberOfAtoms(); n++) {
        tmp1+=mypack.centeredpos[n][b]*vecs[n][a];
      }

      for(unsigned iat=0; iat<getNumberOfAtoms(); iat++) {
        if( normalized ) {
          mypack.addAtomDerivatives( iat, getDisplace()[iat]*mypack.DRotDPos[a][b][iat]*tmp1 );
        } else {
  ...
  Vector v1;
  v1.zero();
  double prefactor = 1. / static_cast<double>( getNumberOfAtoms() );
  for(unsigned n=0; n<getNumberOfAtoms(); n++) {
    v1+=prefactor*matmul(trot,vecs[n]);
  }
  if( normalized ) {
    for(unsigned iat=0; iat<getNumberOfAtoms(); iat++) {
      mypack.addAtomDerivatives( iat, getDisplace()[iat]*(matmul(trot,vecs[iat]) - v1) );
    }
  } else {
    for(unsigned iat=0; iat<getNumberOfAtoms(); iat++) {
      mypack.addAtomDerivatives( iat, (matmul(trot,vecs[iat]) - v1) );
    }
  }
