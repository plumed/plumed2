...
void ReferenceArguments::readArgumentsFromPDB( const PDB& pdb ) {
  ...
  for(unsigned i=0; i<arg_names.size(); ++i) {
    arg_names[i]=pdb.getArgumentNames()[i];
  }
  if( !aref && arg_names.size()==0 ) {
    error("no arguments in input PDB file");
  }

  reference_args.resize( arg_names.size() );
  arg_der_index.resize( arg_names.size() );
  for(unsigned i=0; i<arg_names.size(); ++i) {
    if( !pdb.getArgumentValue(arg_names[i], reference_args[i]) ) {
      error("argument " + arg_names[i] + " was not set in pdb input");
    }
    arg_der_index[i]=i;
  }
  ...
  if( hasweights ) {
    ...
    sqrtweight.resize( arg_names.size() );
    for(unsigned i=0; i<reference_args.size(); ++i) {
      if( !pdb.getArgumentValue("sigma_" + arg_names[i], weights[i]) ) {
        error("value sigma_" + arg_names[i] + " was not set in pdb input");
      }
...
void ReferenceArguments::getArgumentRequests( std::vector<std::string>& argout, bool disable_checks ) {
  ...
  if( argout.size()==0 ) {
    ...
    for(unsigned i=0; i<arg_names.size(); ++i) {
      if(!disable_checks) {
        if( argout[i]!=arg_names[i] ) {
          error("found mismatched arguments in pdb frames");
        }
        arg_der_index[i]=i;
      } else {
        bool found=false;
        for(unsigned j=0; j<arg_names.size(); ++j) {
          if( argout[j]==arg_names[i] ) {
            found=true;
            arg_der_index[i]=j;
            break;
          }
  ...
  if( hasmetric ) {
    ...
    for(unsigned i=0; i<reference_args.size(); ++i) {
      for(unsigned j=i; j<reference_args.size(); ++j) {
        ...
        trig_metric[k]=metric(i,j);
        k++;
      }
    }
  } else {
    if( trig_metric.size()!=reference_args.size() ) {
      trig_metric.resize( reference_args.size() );
    }
    for(unsigned i=0; i<reference_args.size(); ++i) {
      trig_metric[i]=weights[i];
    }
  }
  return trig_metric;
}
...
double ReferenceArguments::calculateArgumentDistance( const std::vector<Value*> & vals, const std::vector<double>& arg,
  ...
  double r=0;
  std::vector<double> arg_ders( vals.size() );
  if( hasmetric ) {
    for(unsigned i=0; i<reference_args.size(); ++i) {
  ...
  if(!squared) {
    r=std::sqrt(r);
    double ir=1.0/(2.0*r);
    for(unsigned i=0; i<arg_ders.size(); ++i) {
      myder.setArgumentDerivatives( i, arg_ders[i]*ir );
    }
  } else {
    for(unsigned i=0; i<arg_ders.size(); ++i) {
      myder.setArgumentDerivatives( i, arg_ders[i] );
    }
  }
  return r;
}
...
void ReferenceArguments::displaceReferenceArguments( const double& weight, const std::vector<double>& displace ) {
  ...
  for(unsigned i=0; i<displace.size(); ++i) {
    reference_args[i] += weight*displace[i];
  }
}
