...
void MoleculeOrientation::normalizeVector( std::vector<double>& vals ) const {
    for(unsigned i=0; i<nvectors; ++i) {
        ...
        double inorm = 1.0;
        if( norm>epsilon ) inorm = 1.0 / norm;
...
void MoleculeOrientation::normalizeVectorDerivatives( MultiValue& myvals ) const {
    ...
    for(unsigned ivec=0; ivec<nvectors; ++ivec) {
        double v=0;
        for(unsigned jcomp=0; jcomp<3; ++jcomp) v += myvals.get( 2+3*ivec+jcomp )*myvals.get( 2+3*ivec+jcomp );
        v=sqrt(v);
        weight[ivec]=1.0;
        wdf[ivec]=1.0;
        if( v>epsilon ) {
    ...
    for(unsigned j=0; j<myvals.getNumberActive(); ++j) {
        ...
        for(unsigned ivec=0; ivec<nvectors; ++ivec) {
            double comp2=0.0;
            for(unsigned jcomp=0; jcomp<3; ++jcomp) comp2 += myvals.get(2+3*ivec+jcomp)*myvals.getDerivative( 2+3*ivec+jcomp, jder );
