...
double VectorMultiColvar::compute( const unsigned& taskIndex, multicolvar::AtomValuePack& myatoms ) const {
  ...
  for(unsigned i=0; i<ncomponents; ++i) {
    norm += myatoms.getValue(2+i)*myatoms.getValue(2+i);
  }
  norm=sqrt(norm);

  if( !doNotCalculateDerivatives() ) {
    double inorm = 1.0 / norm;
    std::vector<double> dervec( ncomponents );
    for(unsigned i=0; i<ncomponents; ++i) {
      dervec[i] = inorm*myatoms.getValue(2+i);
    }
    ...
    for(unsigned j=0; j<myvals.getNumberActive(); ++j) {
      ...
      for(unsigned i=0; i<ncomponents; ++i) {
        myvals.addDerivative( 1, jder, dervec[i]*myvals.getDerivative( 2+i, jder ) );
      }
    }
  }
...
void VectorMultiColvar::normalizeVectorDerivatives( MultiValue& myvals ) const {
  ...
  for(unsigned j=0; j<myvals.getNumberActive(); ++j) {
    ...
    for(unsigned jcomp=2; jcomp<myvals.getNumberOfValues(); ++jcomp) {
      comp2 += myvals.get(jcomp)*myvals.getDerivative( jcomp, jder );
    }
    for(unsigned jcomp=2; jcomp<myvals.getNumberOfValues(); ++jcomp) {
