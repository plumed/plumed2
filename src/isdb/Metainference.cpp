...
Metainference::Metainference(const ActionOptions&ao):
  ...
  if(!getRestart()) {
    average_weights_.resize(nsel, std::vector<double> (nrep_, 1./static_cast<double>(nrep_)));
  } else {
    average_weights_.resize(nsel, std::vector<double> (nrep_, 0.));
  }
...
void Metainference::get_sigma_mean(const unsigned iselect, const double weight, const double norm, const double neff, const std::vector<double> &mean) {
  ...
  if(do_optsigmamean_>0) {
    ...
    if(master) {
      for(unsigned i=0; i<narg; ++i) {
        sigma_mean2_now[i] = weight*(getArgument(i)-mean[i])*(getArgument(i)-mean[i]);
      }
      if(nrep_>1) {
...
void Metainference::replica_averaging(const double weight, const double norm, std::vector<double> &mean, std::vector<double> &dmean_b) {
  if(master) {
    for(unsigned i=0; i<narg; ++i) {
      mean[i] = weight/norm*getArgument(i);
    }
    if(nrep_>1) {
  ...
  for(unsigned i=0; i<narg; ++i) {
    dmean_b[i] = weight/norm/kbt_*(getArgument(i)-mean[i])/static_cast<double>(average_weights_stride_);
  }
