...
private:
  ...
  ActionWithValue* logweights_; // weights to use if reweighting averages
  ...
  value_force2_(NULL)
{
  ...
  if(wvalues.size() > 1) {
    ...
    logweights_ = dynamic_cast<ActionWithValue*> (wvalues[0]->getPntrToAction());
    b_weights_ = true;
  }
...
void EDS::update_statistics()  {
  ...
  if(b_weights_) {
    w = logweights_->copyOutput(0)->get();
    if(max_logweight_ < w) {
