    ...
    last_step_warn_grid(0)
{
    ...
    if(grid_) {
        ...
        if(!sparsegrid) {
            BiasGrid_.reset(new Grid(funcl,getArguments(),gmin,gmax,gbin,spline,true));
        }
        else {
            BiasGrid_.reset(new SparseGrid(funcl,getArguments(),gmin,gmax,gbin,spline,true));
        }
    ...
    for(const auto & p : actionSet) if(dynamic_cast<MetaD*>(p.get())) {
            concurrent=true;
            break;
        }
...
// in this case we have no bias, so reweight factor is 0.0
    ...
    double sum1=0.0;
    double sum2=0.0;
    double afactor = biasf_ / (kbt_*(biasf_-1.0));
    double afactor2 = 1.0 / (kbt_*(biasf_-1.0));
    unsigned rank=comm.Get_rank(), stride=comm.Get_size();
    for(unsigned i=rank; i<ntotgrid; i+=stride) {
        t_index[0]=(i%rewf_grid_[0]);
        unsigned kk=i;
        for(unsigned j=1; j<ncv-1; ++j) {
            kk=(kk-t_index[j-1])/rewf_grid_[i-1];
            t_index[j]=(kk%rewf_grid_[i]);
        }
...
void MetaD::updateFrequencyAdaptiveStride() {
    ...
    if(mean_acc >= fa_min_acceleration_) {
        if(tmp_stride > current_stride) {
            current_stride = tmp_stride;
        }
    }
