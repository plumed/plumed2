...
void MetaD::updateNlist()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned k=0; k<hills_.size(); k++)
    {
      ...
      for(unsigned i=0; i<getNumberOfArguments(); i++)
      {
        const double d=difference(i,getArgumentScalar(i),hills_[k].center[i])/hills_[k].sigma[i];
        dist2+=d*d;
      }
  ...
  for(unsigned i=0; i<getNumberOfArguments(); i++) nlist_center_[i]=getArgumentScalar(i);
  ...
  for(unsigned k=0; k<nlist_hills_.size(); k++)
  {
    for(unsigned i=0; i<getNumberOfArguments(); i++)
    {
      const double d=difference(i,getArgumentScalar(i),nlist_hills_[k].center[i]);
      dev2[i]+=d*d;
    }
  }
  for(unsigned i=0; i<getNumberOfArguments(); i++) {
