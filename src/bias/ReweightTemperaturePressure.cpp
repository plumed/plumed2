...
double ReweightTemperaturePressure::getLogWeight() {
  double energy=0.0; for(unsigned i=0; i<myenergy.size(); ++i) energy+=getArgumentScalar(i);
  double volume=0.0; for(unsigned i=0; i<myvol.size(); ++i) volume+=getArgumentScalar(myenergy.size()+i);
  // 4 possible cases
  // Case 1) Reweight from T to T' with V=const (canonical)
