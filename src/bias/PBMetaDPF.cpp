...
This action activate Parallel Bias Metadynamics (PBMetaDPF_base10) \cite pbmetad, a version of metadynamics \cite metad in which
...
The PBMetaDPF_base10 bias potential is defined by:
...
Currently, only a subset of the \ref METAD options are available in PBMetaDPF_base10.

The bias potentials can be stored on a grid to increase performances of long PBMetaDPF_base10 simulations.
You should
...
variant of PBMetaD_base10 the heights of the Gaussian hills are scaled at each step by the
...
The following input is for PBMetaDPF_base10 calculation using as
collective variables the distance between atoms 3 and 5
and the distance between atoms 2 and 4. The value of the CVs and
the PBMetaDPF_base10 bias potential are written to the COLVAR file every 100 steps.
\plumedfile
...
private:
  ...
  int ip_n_; // initialize number of independent particles
  std::vector<int> ips_; //std::vector length of arguments that holds which ip# each cv belongs in
  std::vector<int> iplen_; // std::vector length of ip_n which stores how many cvs belong in each family
  std::vector<Value*> iphold_; // std::vector length of ip_n which stores a pointer to the first argument fed to each family
  // multiple walkers
  int mw_n_;
  std::string mw_dir_;
  int mw_id_;
...
public:
  explicit PBMetaDPF(const ActionOptions&);
...
PLUMED_REGISTER_ACTION(PBMetaDPF, "PBMETADPF")

void PBMetaDPF::registerKeywords(Keywords& keys) {
  ...
  keys.add("optional","PF_N", "number of independent particles"); // read in number of PF CF
...
PBMetaDPF::PBMetaDPF(const ActionOptions& ao):
  PLUMED_BIAS_INIT(ao),
  kbt_(0.0),
  stride_(0),
  welltemp_(false),
  ...
  ip_n_(0)
{
  ...
  // Identical Particles CF
  parse("PF_N",ip_n_);
  if (ip_n_> 0){
     ips_.resize(getNumberOfArguments());
     iplen_.resize(ip_n_);
     iphold_.resize(ip_n_);
     for(unsigned cfu=0; cfu <ip_n_; cfu++) {
  	std::vector<Value*> carg;
        parseArgumentList("PF",cfu+1,carg);
  	log.printf("Identified Identical Particles");
        iplen_[cfu]=carg.size(); //store number of cvs for each family.
  	 for(unsigned i=0; i<carg.size(); i++) {
  	    log.printf(" %s",carg[i]->getName().c_str());
  		 for(unsigned ru=0; ru<getNumberOfArguments(); ru++){
  		    if (carg[i]->getName().c_str() == getPntrToArgument(ru)->getName()){
                       ips_[ru]=cfu+1; //store the ip# for each cv
  		if(iphold_[cfu] == 0){
 		iphold_[cfu]=getPntrToArgument(ru) ; } // if this is the first argument in the family, store a pointer for it (this is for HILLS & GRID files)
                       log.printf(" %d",ips_[ru]) ;                                                                                        }                                                                                                                 }
                 log.printf("\n");
         }
      }
   }
  ...
  for(int oo=0; oo < hillsfname_.size(); oo++)
  { log.printf(hillsfname_[oo].c_str());
     log.printf(iphold_[oo]->getName().c_str());}

  if(hillsfname_.size()==0) {
    for(unsigned i=0; i<ip_n_; i++) hillsfname_.push_back("HILLS.PF" + std::to_string(i + 1)); // number of hills files corresponds to number of ip_n now. CF
  }
  if(hillsfname_.size()!=ip_n_) { //number of hills files corresponds to number of ip_n now. CF
...
// ALL GOOD JIMKM
  ...
  if(gridfilenames_.size()==0 && wgridstride_ > 0) {
    for(unsigned i=0; i<ip_n_; i++) gridfilenames_.push_back("GRID.PF"+std::to_string(i+1));
  }
  ...
  // Grid Stuff
  // we set grid argument sizes to ip_n instead of arguments. hillsfname.size() should also work. CF 
  std::vector<std::string> gmin(ip_n_);
  parseVector("GRID_MIN",gmin);
  if(gmin.size()!=ip_n_ && gmin.size()!=0) error("not enough values for GRID_MIN");
  std::vector<std::string> gmax(ip_n_);
  parseVector("GRID_MAX",gmax);
  if(gmax.size()!=ip_n_ && gmax.size()!=0) error("not enough values for GRID_MAX");
  std::vector<unsigned> gbin(ip_n_);
  std::vector<double>   gspacing;
  parseVector("GRID_BIN",gbin);
  if(gbin.size()!=ip_n_ && gbin.size()!=0) error("not enough values for GRID_BIN");
  parseVector("GRID_SPACING",gspacing);
  if(gspacing.size()!=ip_n_ && gspacing.size()!=0) error("not enough values for GRID_SPACING");
  ...
  if(gmin.size()!=0) {
    if(gbin.size()==0 && gspacing.size()==0) {
      if(adaptive_==FlexibleBin::none) {
        ...
        //plumed_assert(sigma0_.size()==getNumberOfArguments()); CF We commented this out. we ignored adaptive sigma
        gspacing.resize(ip_n_);
    ...
    if(gbin.size()==0) gbin.assign(ip_n_,1);
    if(gspacing.size()!=0) for(unsigned i=0; i<ip_n_; i++) {
        double a,b;
  ...
  // we set interval to match ip_n, but for the most part largely ignored it. CF
  doInt_.resize(ip_n_,false);
  // Interval keyword
  ...
  if(lowI_.size()!=0 && lowI_.size()!=ip_n_) error("check number of argument of INTERVAL");
  for(unsigned i=0; i<lowI_.size(); ++i) {
    if(uppI_[i]<lowI_[i]) error("The Upper limit must be greater than the Lower limit!");
    if(iphold_[i]->isPeriodic()) warning("INTERVAL is not used for periodic variables");
    else doInt_[i]=true;
  }
  ...
  if(mw_n_>1) { //we ignored this as we we are not tackling MW yet CF
...
// we ignored this block since we are not tackling intervals yet. CF
  ...
  hills_.resize(ip_n_); //number of hills files should be ip_n. CF
  ...
  if(grid_) {
    // check for mesh and sigma size. Instead of pointing to arguments, we point to the families. CF
    for(unsigned i=0; i<getNumberOfArguments(); i++) {
      double a,b;
      Tools::convert(gmin[ips_[i]-1],a);
      Tools::convert(gmax[ips_[i]-1],b);
      double mesh=(b-a)/((double)gbin[ips_[i]-1]);
      if(adaptive_==FlexibleBin::none) {
    ...
    for(unsigned i=0; i<iphold_.size(); ++i) { //Use the arguments in ip_hold as the dummy pointer to interacting with gridfiles. CF
      ...
      if(gridreadfilenames_.size()>0) {
        ...
        if(iphold_[i]->isPeriodic() != BiasGrid_->getIsPeriodic()[0]) {
      ...
      } //TACKLE GRIDS, REWRITE PBMetaDPF_base10NCTION CREATE
      BiasGrids_.emplace_back(std::move(BiasGrid_));
    }
  }
//COMPILE & RAN, BLANK FILES GENERATED

// creating vector of ifile* for hills reading
// open all files at the beginning and read Gaussians if restarting
log.printf("Entering into creating ifiles\n");
  ifilesnames_.resize(ip_n_); //GRABTEST. files corresponds to ip_n_, not arguments anymore. CF
  ifiles_.resize(ip_n_); //GRABTEST. files corresponds to ip_n_, not arguments anymore.
  for(int j=0; j<mw_n_; ++j) {
    for(unsigned i=0; i<getNumberOfArguments(); ++i) {
      //unsigned k=j*hillsfname.size()+i; NEED FOR MW, but we are ignoring MW. CF
      unsigned cfuk=ips_[i]-1; //We are iterating over number of arguments, but want them to point to the hillsfile of the families so we convert to index of hills file. CF
      log.printf(" %d\n",cfuk);
      std::string fname;
      if(mw_dir_!="") {
        if(mw_n_>1) {
          std::stringstream out; out << j;
          fname = mw_dir_+"/"+hillsfname_[cfuk]+"."+out.str();
        } else if(walkers_mpi_) {
          fname = mw_dir_+"/"+hillsfname_[cfuk];
        } else {
          fname = hillsfname_[cfuk];
        }
      } else {
        if(mw_n_>1) {
          std::stringstream out; out << j;
          fname = hillsfname_[cfuk]+"."+out.str();
        } else {
          fname = hillsfname_[cfuk];
        }
      } bool repeating_files;
	 repeating_files=false;
	log.printf("Working through file %s\n", hillsfname_[cfuk].c_str()); //PRINTING THIS as a CHECK CF
      for(int ind=0; ind < ifilesnames_.size(); ind++) {
	if (ifilesnames_[ind] == fname) // Don't want to reinitialize a family we already initialized CF
		repeating_files=true;} // for loop of truth for the win!!!
	if (!repeating_files) {
      ifiles_[cfuk] = Tools::make_unique<IFile>();
      IFile *ifile = ifiles_[cfuk].get();
      ifile->link(*this);
    ifilesnames_[cfuk]=fname;
      //log.printf("  CHECK from %s\n:",ifilesnames[cfuk].c_str());
      if(ifile->FileExist(fname)) {
        ifile->open(fname);
//	log.printf("  CHECK from %s\n:",ifilesnames[cfuk].c_str());
        if(getRestart()&&!restartedFromGrid) {
          log.printf("  CHECK from %s\n:", ifilesnames_[cfuk].c_str());
          log.printf("  Restarting from %s:",ifilesnames_[cfuk].c_str());
          readGaussians(i,ifiles_[ips_[i] - 1].get()); // should be fine. CF
        }
        ifiles_[ips_[i] - 1]->reset(false);
        //log.printf("  CHECK %s:",ifilesnames[cfuk].c_str());
        // close only the walker own hills file for later writing
        if(j==mw_id_) ifiles_[ips_[i] - 1]->close();
      } else {
	...
	} // !end of if repeating_file
    } // Compiled, creating correct hills but blank
  }

  comm.Barrier();
  if(comm.Get_rank()==0 && walkers_mpi_) multi_sim_comm.Barrier();
log.printf("open hills for writing\n");
  ...
  for(unsigned i=0; i<hillsfname_.size(); ++i) {
    ...
    if(walkers_mpi_) {
      ...
      comm.Bcast(r,0);
      if(r>0) ifilesnames_[i]="/dev/null";
      ofile->enforceSuffix("");
    }
    ...
    ofile->setupPrintValue( iphold_[i] ); //assuming cvs in the same family have the same periodicity and boundaries.
    // push back
    hillsOfiles_.emplace_back(std::move(ofile));
  }
for(int rfu=0;rfu<hillsOfiles_.size();rfu++){
	}//log.printf(hillsOfiles_[rfu].c_str());}
log.printf("finished hills\n now dump grid to files\n");
  // Dump grid to files
  if(wgridstride_ > 0) {
    for(unsigned i = 0; i < gridfilenames_.size(); ++i) {
      auto ofile=Tools::make_unique<OFile>();
...
log.printf("done with dumping grids\n");
...
void PBMetaDPF::readGaussians(unsigned iarg, IFile *ifile)
{
  ...
  std::vector<Value> tmpvalues; //want to feed the dummy variably for each hills file as the argument printed in hillsfile column headers. CF
  tmpvalues.push_back( Value( this, iphold_[ips_[iarg]-1]->getName(), false ) ); //8/24-9:20PM

  while(scanOneHill(iarg,ifile,tmpvalues,center,sigma,height,multivariate)) {
    ;
    nhills++;
    if(welltemp_) {height*=(biasf_-1.0)/biasf_;}
    addGaussian(ips_[iarg]-1, Gaussian(center,sigma,height,multivariate)); //feed pointer to hills file not argument CF
  }
  log.printf("      %d Gaussians read\n",nhills);
}

void PBMetaDPF::writeGaussian(unsigned iarg, const Gaussian& hill, OFile *ofile)
{
  ofile->printField("time",getTimeStep()*getStep());
  ofile->printField(iphold_[ips_[iarg]-1],hill.center[0]); //print dummy variable in ihold, not the argument. CF

  ofile->printField("kerneltype","gaussian");
  if(hill.multivariate) { //we are noit doing multivariate at this point (Not possible for PBMetad??) CF
    ...
    ofile->printField("sigma_"+iphold_[ips_[iarg]-1]->getName(),hill.sigma[0]); //print dummy variable in ihold, not the argument. CF
  }
...
void PBMetaDPF::addGaussian(unsigned iarg, const Gaussian& hill)
{ // Should all be fine, the inputs to this function were adjusted accordingly. CF 
...
std::vector<unsigned> PBMetaDPF::getGaussianSupport(unsigned iarg, const Gaussian& hill)
{ // should all be fine CF
...
double PBMetaDPF::getBiasAndDerivatives(unsigned iarg, const std::vector<double>& cv, double* der)
{ // rerouted pointer to hills files. CF
  double bias=0.0;
  if(!grid_) {
    unsigned stride=comm.Get_size();
    unsigned rank=comm.Get_rank();
    for(unsigned i=rank; i<hills_[ips_[iarg]-1].size(); i+=stride) {
      bias += evaluateGaussian(iarg,cv,hills_[ips_[iarg]-1][i],der);  // rerouted pointer to hills files. CF
    }
    comm.Sum(bias);
    ...
    if(der) {
      ...
      bias = BiasGrids_[ips_[iarg]-1]->getValueAndDerivatives(cv,vder);
      der[0] = vder[0];
    } else {
      bias = BiasGrids_[ips_[iarg]-1]->getValue(cv);
    }
  }
...
double PBMetaDPF::evaluateGaussian(unsigned iarg, const std::vector<double>& cv, const Gaussian& hill, double* der)
{
  double bias=0.0;
...
void PBMetaDPF::update()
{
  ...
  if(nowAddAHill) {
    ...
    // MPI Multiple walkers: share hills and add them all. We ignored MW sections
    if(walkers_mpi_) {
	...
	// Changed for MW. AP
          addGaussian(ips_[i]-1, newhill);
          writeGaussian(i, newhill, hillsOfiles_[ips_[i]-1].get());
        }
      }
      ...
      for(unsigned i=0; i<getNumberOfArguments(); ++i) {
        ...
        addGaussian(ips_[i]-1, newhill);   // rerouted pointer to family instead of arguments. CF
        writeGaussian(i, newhill, hillsOfiles_[ips_[i]-1].get());  // rerouted pointer to hills files. CF
      }
    }
  }
  ...
  // if multiple walkers and time to read Gaussians.  We ignore MW sections. 
...
bool PBMetaDPF::scanOneHill(unsigned iarg, IFile *ifile, std::vector<Value> &tmpvalues, std::vector<double> &center, std::vector<double> &sigma, double &height, bool &multivariate)
{
  double dummy;
  multivariate=false;
  if(ifile->scanField("time",dummy)) {
    ifile->scanField( &tmpvalues[0] );
    if( tmpvalues[0].isPeriodic() && ! iphold_[ips_[iarg]-1]->isPeriodic() ) { // point to dummy argument in ihold, not arguments fed to PBMetad
      ...
      std::string rmin, rmax; iphold_[ips_[iarg]-1]->getDomain( rmin, rmax ); // point to dummy argument in ihold, not arguments fed to PBMetad
    ...
    if(multivariate) { // we ignore multivariate
      ifile->scanField("sigma_"+getPntrToArgument(iarg)->getName()+"_"+
                       getPntrToArgument(iarg)->getName(),sigma[0]);
      sigma[0] = 1./(sigma[0]*sigma[0]);
    } else {
      ifile->scanField("sigma_"+iphold_[ips_[iarg]-1]->getName(),sigma[0]); // print name of dummy variable in ihold
    }
...
bool PBMetaDPF::checkNeedsGradients()const
{
