...
void ActionVolume::requestAtoms( const std::vector<AtomNumber> & a ) {
  ...
  if( getPntrToComponent(0)->getRank()==0 ) getPntrToComponent(0)->resizeDerivatives( 3*getNumberOfAtoms()+9 );
}
...
void ActionVolume::getNumberOfTasks( unsigned& ntasks ) {
  setupRegions(); ActionWithVector::getNumberOfTasks( ntasks );
}
...
void ActionVolume::calculate() {
  ...
  if( getPntrToComponent(0)->getRank()==0 ) {
    ...
    // Atom position
    Value* v = getPntrToComponent(0); v->set( weight );
    for(unsigned i=0; i<3; ++i ) v->addDerivative( i, wdf[i] );
    // Add derivatives with respect to reference positions
    for(unsigned i=0; i<refders.size(); ++i) {
      for(unsigned j=0; j<3; ++j ) v->addDerivative( 3 + 3*i + j, refders[i][j] );
    }
    // Add virial
    unsigned vbase = 3*getNumberOfAtoms();
    for(unsigned i=0; i<3; ++i) for(unsigned j=0; j<3; ++j) v->addDerivative( vbase + 3*i + j, vir(i,j) );
  } else runAllTasks();
}
