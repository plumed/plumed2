...
The SASA may also be computed using the SASA_HASEL collective variable, which makes us of the algorithm described in (Hasel et al., Tetrahedron Computer Methodology Vol. 1, No. 2, pp. 103-116, 1988). SASA_HASEL is less accurate then SASA_LCPO, but the computation is faster.
...
void SASA_LCPO::registerKeywords(Keywords& keys) {
  ...
  keys.add("optional","DELTAGFILE","a file containing the free energy values for backbone and sidechains. Necessary only if TYPE = TRANSFER");
