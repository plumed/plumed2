...
void ContourFindingBase::performOperationsWithGrid( const bool& from_update ){
  // Find the contour
  findContourSurface();
  // And this looks after the output
  if( mydata ){
      std::vector<double> point( 1 + mygrid->getDimension() );
      ...
      for(unsigned i=0;i<mydata->getNumberOfStoredValues();++i){
          ...
          for(unsigned j=0;j<mygrid->getDimension();++j) of.printf( (" " + fmt_xyz).c_str(), lenunit*point[1+j] ); 
          of.printf("\n");
      }
  } 
}
