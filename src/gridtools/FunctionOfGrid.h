  ...
  ActionWithGrid(ao)
{
  ...
  if( getPntrToArgument(0)->getRank()==0 || !getPntrToArgument(0)->hasDerivatives() ) error("first input to this action must be a grid");
...
void FunctionOfGrid<T>::setupOnFirstStep( const bool incalc ) {
  ...
  if( mygrid.getGridType()=="flat" ) {
    ...
    volume=vv[0]; for(unsigned i=1; i<vv.size(); ++i) volume *=vv[i];
  } else volume=4*pi / static_cast<double>( npoints );
  // This resizes the scalars
  for(int i=0; i<getNumberOfComponents(); ++i) {
    if( getPntrToComponent(i)->getRank()==0 ) getPntrToComponent(i)->resizeDerivatives( npoints );
  }
  if( getName()=="SUM_GRID" ) volume = 1.0;
  // This sets the prefactor to the volume which converts integrals to sums
  myfunc.setup( this ); myfunc.setPrefactor( this, volume );
}
...
void FunctionOfGrid<T>::apply() {
  ...
  if( myfunc.zeroRank() ) { ActionWithVector::apply(); return; }
  ...
  std::vector<double> totv(nscalars,0); Value* outval=getPntrToComponent(0);
  for(unsigned i=0; i<outval->getNumberOfValues(); ++i) {
    nscalars=0;
    for(unsigned j=argstart; j<getNumberOfArguments(); ++j) {
      double fforce = outval->getForce(i);
      if( getPntrToArgument(j)->getRank()==0 ) {
        totv[nscalars] += fforce*outval->getGridDerivative( i, outval->getRank()+j ); nscalars++;
      } else {
