...
GridVessel::GridVessel( const vesselbase::VesselOptions& da ):
  ...
  for(unsigned i=0; i<coordnames.size(); ++i) {
    arg_names[n] = coordnames[i];
    n++;
  }
  for(unsigned i=0; i<compnames.size(); ++i) {
    arg_names[n]=compnames[i];
    n++;
    for(unsigned j=0; j<coordnames.size(); ++j) {
      arg_names[n] = "d" + compnames[i] + "_" + coordnames[j];
      n++;
    }
  }
  pbc.resize( dimension );
  std::vector<std::string> spbc( dimension );
...
void GridVessel::setupFibonacciGrid( const unsigned& np ) {
  ...
  double final_cutoff;
  if( getFibonacciCutoff()<-1 ) {
    final_cutoff=-1;
  } else {
    final_cutoff = std::cos( std::acos( getFibonacciCutoff() ) + std::acos( min ) );
  }
...
void GridVessel::getIndices( const std::vector<double>& point, std::vector<unsigned>& indices ) const {
  ...
  for(unsigned i=0; i<dimension; ++i) {
    ...
    if( pbc[i] ) {
      ...
      for(unsigned j=1; j<point.size(); ++j) {
        Tools::convert( point[j], num );
        pp += ", " + num;
      }
...
void GridVessel::setValueAndDerivatives( const unsigned& ipoint, const unsigned& jelement, const double& value, const std::vector<double>& der ) {
  ...
  setGridElement( ipoint, jelement, value );
  for(unsigned i=0; i<der.size(); ++i) {
    setGridElement( ipoint, jelement+1+i, der[i] );
  }
}
...
void GridVessel::calculate( const unsigned& current, MultiValue& myvals, std::vector<double>& buffer, std::vector<unsigned>& der_list ) const {
  ...
  for(unsigned i=0; i<nper; ++i) {
    buffer[bufstart + nper*current + i] += myvals.get(i+1);
  }
}
...
// loop over neighbors
  for(unsigned int ipoint=0; ipoint<n_neigh; ++ipoint) {
    ...
    for(unsigned j=0; j<dimension; ++j) {
      dder[j] = getGridElement( neigh[ipoint], ind*(1+dimension) + 1 + j );
    }
...
bool GridVessel::applyForce( std::vector<double>& fforces ) {
  ...
  for(unsigned i=0; i<finalForces.size(); ++i) {
    fforces[i]=finalForces[i];
  }
  wasforced=false;
  return true;
}
