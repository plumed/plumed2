...
KDE::KDE(const ActionOptions&ao):
  ...
  fixed_width(false)
{
  std::vector<unsigned> shape( getNumberOfArguments() ); center.resize( getNumberOfArguments() );
  ...
  if( getName()=="KDE" ) {
    ...
    if( kerneltype!="DISCRETE" ) {
      std::string bandwidth; std::vector<std::string> bwidths; parseVector("BANDWIDTH",bwidths);
      if( bwidths.size()>0 ) {
  ...
  if( weight_str.size()>0 ) {
    ...
    if( weight_str.size()==2 ) {
      ...
      if( weight_args[1]->getRank()==1 && weight_args[1]->getNumberOfValues()!=shape.size() ) error("size of bandwidth vector is incorrect");
      if( weight_args[1]->getRank()>2 ) error("bandwidths cannot have rank greater than 2");
      bwargno=args.size()-1; log.printf("  bandwidths are taken from : %s \n", weight_str[1].c_str() );
    } else if( !hasheight ) {
      if( weight_args[0]->getRank()==1 && weight_args[0]->getNumberOfValues()!=shape.size() ) error("size of bandwidth vector is incorrect");
      if( weight_args[0]->getRank()>2 ) error("bandwidths cannot have rank greater than 2");
      bwargno=args.size()-1; log.printf("  bandwidths are taken from : %s \n", weight_str[0].c_str() );
    } else if ( weight_str.size()==1 ) {
      log.printf("  quantities used for weights are : %s \n", weight_str[0].c_str() );
    } else error("only one scalar/vector/matrix should be input to HEIGHTS");
  ...
  if( getName()=="KDE" ) {
    ...
    parse("CONCENTRATION",von_misses_concentration); fixed_width=true;
    ...
    // Setup the grid
    shape[0]=nbins; shape[1]=shape[2]=1;
  }
...
std::vector<std::string> KDE::getGridCoordinateNames() const {
  ...
  for(unsigned i=0; i<names.size(); ++i) names[i] = getPntrToArgument(i)->getName();
  return names;
}
...
void KDE::areAllTasksRequired( std::vector<ActionWithVector*>& task_reducing_actions ) {
  if( numberOfKernels==1 || (hasheight && getPntrToArgument(gridobject.getDimension())->getRank()>0) ) task_reducing_actions.push_back(this);
}
...
void KDE::getNumberOfTasks( unsigned& ntasks ) {
  ...
  hh = 1.0; if( hasheight ) hh = getPntrToArgument(gridobject.getDimension())->get();
  for(unsigned i=0; i<center.size(); ++i) center[i]=getPntrToArgument(i)->get();
  ...
  if( kerneltype=="DISCRETE" ) {
    num_neigh=1; neighbors.resize(1);
    for(unsigned i=0; i<center.size(); ++i) center[i] += 0.5*gridobject.getGridSpacing()[i];
...
int KDE::checkTaskStatus( const unsigned& taskno, int& flag ) const {
  ...
  for(unsigned i=0; i<num_neigh; ++i) {
    if( taskno==neighbors[i] ) return 1;
  }
  return 0;
}
  ...
  if( numberOfKernels==1 ) {
    ...
    if( getName()=="KDE" ) {
      if( kerneltype=="DISCRETE" ) {
        ...
        double val=hh; std::size_t dd = kerneltype.find("-bin");
        ...
        for(unsigned j=0; j<args.size(); ++j) der[j] *= val; newval=val;
      } else {
...
void KDE::retrieveArgumentsAndHeight( const MultiValue& myvals, std::vector<double>& args, double& height ) const {
  ...
  if( hasheight && getPntrToArgument(args.size())->getRank()==0 ) height = getPntrToArgument( args.size() )->get();
  else if( hasheight ) height = getPntrToArgument( args.size() )->get( myvals.getTaskIndex() );
}
...
void KDE::gatherStoredValue( const unsigned& valindex, const unsigned& code, const MultiValue& myvals,
  ...
  if( kerneltype!="DISCRETE" ) gridobject.getNeighbors( args, nneigh, num_neigh, neighbors );
