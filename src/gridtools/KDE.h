...
void KDEHelper<K,P,G>::read( KDEHelper<K,P,G>& func, 
  ...
  for(unsigned i=1; i<action->getNumberOfArguments(); ++i) {
      if( (action->getPntrToArgument(0))->getNumberOfValues()==0 && (action->getPntrToArgument(i))->isConstant() ) continue;
...
void KDEHelper<K,P,G>::readKernelParameters( std::string& value, ActionWithArguments* action, const std::string& outlab, bool rerequestargs ) {
  ...
  if( !av ) {
      ...
      for(unsigned i=1; i<(action->getPntrToArgument(0))->getNumberOfValues(); ++i) vals += "," + value;
      ...
      if( dot!=std::string::npos ) {
          ...
          if( av->getNumberOfComponents()>1 ) action->error("problem reading argument " + value );
