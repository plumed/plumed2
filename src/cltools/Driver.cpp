...
template<typename real>
string Driver<real>::description()const {
        ...
        if( first_step || rnd.U01()>0.5) {
            if(debug_pd) {
                ...
                if(intracomm.Get_rank()==0) {
                    ...
                    fprintf(out,"DRIVER: ");
                    for(int i=0; i<npe; i++) fprintf(out,"%d ",loc[i]);
                    printf("\n");
                    fprintf(out,"DRIVER: ");
                    for(int i=0; i<npe; i++) fprintf(out,"%d ",start[i]);
                    printf("\n");
                }
                ...
                for(int i=0; i<natoms; ++i) {
                    ...
                    int n;
                    for(n=0; n<npe; n++) if(n+1>r)break;
                    plumed_assert(n<npe);
                    if(n==rank) {
