...
string CLToolSumHills::description()const {
...
//  }
    ...
    if(dohills) {
        addme="HILLSFILES=";
        for(unsigned i=0; i<hillsFiles.size()-1; i++)addme+=hillsFiles[i]+",";
    ...
    if(grid_check==2) {
        addme="GRID_MAX=";
        for(unsigned i=0; i<(ncv-1); i++)addme+=gmax[i]+",";
        addme+=gmax[ncv-1];
        actioninput.push_back(addme);
        addme="GRID_MIN=";
        for(unsigned i=0; i<(ncv-1); i++)addme+=gmin[i]+",";
        addme+=gmin[ncv-1];
        actioninput.push_back(addme);
    }
    if(grid_has_bin) {
        addme="GRID_BIN=";
        for(unsigned i=0; i<(ncv-1); i++)addme+=gbin[i]+",";
        addme+=gbin[ncv-1];
        actioninput.push_back(addme);
    }
    if(grid_has_spacing) {
        addme="GRID_SPACING=";
        for(unsigned i=0; i<(ncv-1); i++)addme+=gspacing[i]+",";
