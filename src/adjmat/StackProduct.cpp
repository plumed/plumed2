...
class StackProduct : public ActionWithMatrix {
private:
  unsigned nderivatives;
  std::vector<bool> stored_arg;
public:
  static void registerKeywords( Keywords& keys );
  explicit StackProduct(const ActionOptions&);
...
PLUMED_REGISTER_ACTION(StackProduct,"STACK_PRODUCT")

void StackProduct::registerKeywords( Keywords& keys ) {
  ActionWithMatrix::registerKeywords(keys); keys.use("ARG");
  ActionWithValue::useCustomisableComponents(keys);
}

StackProduct::StackProduct(const ActionOptions&ao):
Action(ao),
ActionWithMatrix(ao)
{
  ...
  if( nvectors==1 ) {
      ...
      if( getPntrToArgument(n)->getRank()!=1 || getPntrToArgument(n)->hasDerivatives() ) error("first argument to this action should be a vector");
  ...
  for(unsigned i=1; i<getNumberOfArguments(); ++i) { 
      ...
      componentIsNotPeriodic( getPntrToArgument(i)->getName() ); 
  }
  std::string headstr=getFirstActionInChain()->getLabel();
  nderivatives = buildArgumentStore(0); stored_arg.resize( getNumberOfArguments() ); 
  for(unsigned i=0; i<getNumberOfArguments(); ++i) stored_arg[i] = getPntrToArgument(i)->ignoreStoredValue( headstr ); 
}

unsigned StackProduct::getNumberOfDerivatives() {
  return nderivatives;
}

void StackProduct::setupForTask( const unsigned& task_index, std::vector<unsigned>& indices, MultiValue& myvals ) const {
...
void StackProduct::performTask( const std::string& controller, const unsigned& index1, const unsigned& index2, MultiValue& myvals ) const {
...
void StackProduct::runEndOfRowJobs( const unsigned& ind, const std::vector<unsigned> & indices, MultiValue& myvals ) const {
...
void StackProduct::updateAdditionalIndices( const unsigned& ostrn, MultiValue& myvals ) const {
