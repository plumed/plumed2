...
void OldAdjacencyMatrixBase::registerKeywords( Keywords& keys ) {
  multicolvar::MultiColvarBase::registerKeywords( keys );
  keys.remove("LOWMEM"); keys.use("HIGHMEM");
}

OldAdjacencyMatrixBase::OldAdjacencyMatrixBase(const ActionOptions& ao):
  Action(ao),
  MultiColvarBase(ao),
  connect_id(0),
...
void OldAdjacencyMatrixBase::parseConnectionDescriptions( const std::string& key, const bool& multiple, const unsigned& nrow_t ) {
...
unsigned OldAdjacencyMatrixBase::getSizeOfInputVectors() const {
...
unsigned OldAdjacencyMatrixBase::getNumberOfNodeTypes() const {
  unsigned size=mybasemulticolvars.size();
  if( size==0 ) return 1;
  return size;
}

void OldAdjacencyMatrixBase::retrieveTypeDimensions( unsigned& nrows, unsigned& ncols, unsigned& ntype ) const {
...
void OldAdjacencyMatrixBase::finishMatrixSetup( const bool& symmetric, const std::vector<AtomNumber>& all_atoms ) {
  std::string param;
...
void OldAdjacencyMatrixBase::readMaxTwoSpeciesMatrix( const std::string& key0, const std::string& key1, const std::string& key2, const bool& symmetric ) {
  std::vector<AtomNumber> all_atoms; readTwoGroups( key0, key1, key2, all_atoms );
  finishMatrixSetup( symmetric, all_atoms );
}

void OldAdjacencyMatrixBase::readMaxThreeSpeciesMatrix( const std::string& key0, const std::string& key1, const std::string& key2, const std::string& keym, const bool& symmetric ) {
...
void OldAdjacencyMatrixBase::recalculateMatrixElement( const unsigned& myelem, MultiValue& myvals ) {
