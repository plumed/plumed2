  ...
  Function(ao)
{
  ...
  prefactor = 1.0 / static_cast<double>( nvals );
  addValueWithDerivatives();
  checkRead();
}

void Mean::calculateFunction( const std::vector<double>& args, MultiValue& myvals ) const {
  plumed_dbg_assert( args.size()==1 ); setValue( 0, prefactor*args[0], myvals ); addDerivative( 0, 0, prefactor, myvals );
}
