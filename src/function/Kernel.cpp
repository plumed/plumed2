  ...
  ActionShortcut(ao)
{
  ...
  if( argnames.size()==0 ) {
      for(unsigned i=1;; ++i) {
          ...
          argnames.push_back( argn);
      }
  } 
  // Now sort out the parameters
  double weight; std::string fname; parse("REFERENCE",fname); bool usemahalanobis=false;
  if( fname.length()>0 ) {
      ...
      for(unsigned line=0;line<number;++line) {
          for(unsigned i=0;i<argnames.size();++i) { 
              std::string val; ifile.scanField(argnames[i], val);
              if( line==number-1 ) readInputLine( getShortcutLabel() + "_" + fixArgumentDot(argnames[i]) + "_ref: CONSTANT_VALUE VALUES=" + val );
          }
          if( ifile.FieldExist("sigma_" + argnames[0]) ) {
              std::string varstr;
              for(unsigned i=0;i<argnames.size();++i) {
                  std::string val; ifile.scanField("sigma_" + argnames[i], val);
                  if( i==0 ) varstr = val; else varstr += "," + val;
              }
              if( line==number-1 ) readInputLine( getShortcutLabel() + "_var: CONSTANT_VALUE VALUES=" + varstr );
          } else {
              std::string varstr, nvals; Tools::convert( argnames.size(), nvals ); usemahalanobis=(argnames.size()>1);
              for(unsigned i=0; i<argnames.size(); ++i) {
                  for(unsigned j=0; j<argnames.size(); j++) {
                      std::string val; ifile.scanField("sigma_" +argnames[i] + "_" + argnames[j], val );
                      if(i==0 && j==0 ) varstr = val; else varstr += "," + val;
                  }
              }
              if( line==number-1 ) {
                  if( !usemahalanobis ) readInputLine( getShortcutLabel() + "_var: CONSTANT_VALUE VALUES=" + varstr );
                  else readInputLine( getShortcutLabel() + "_cov: CONSTANT_VALUE NCOLS=" + nvals + " NROWS=" + nvals + " VALUES=" + varstr );
              }
          }
      ...
      // Create actions to hold the position of the center
      std::vector<std::string> center(argnames.size()); parseVector("CENTER",center); 
      for(unsigned i=0; i<argnames.size(); ++i) readInputLine( getShortcutLabel() + "_" + fixArgumentDot(argnames[i]) + "_ref: CONSTANT_VALUE VALUES=" + center[i] );
      std::vector<std::string> sig; parseVector("SIGMA",sig); 
      if( sig.size()==0 ) {
          // Create actions to hold the covariance
          std::string cov; parse("COVAR",cov); usemahalanobis=(argnames.size()>1);
          if( !usemahalanobis ) {
            readInputLine( getShortcutLabel() + "_var: CONSTANT_VALUE VALUES=" + cov );
          } else { 
            std::string nvals; Tools::convert( argnames.size(), nvals ); 
            readInputLine( getShortcutLabel() + "_cov: CONSTANT_VALUE NCOLS=" + nvals + " NROWS=" + nvals + " VALUES=" + cov );
          }
       } else if( sig.size()==argnames.size() ) {
          // And actions to hold the standard deviation
          std::string valstr = sig[0]; for(unsigned i=1; i<sig.size(); ++i) valstr += "," + sig[i];
          readInputLine( getShortcutLabel() + "_sigma: CONSTANT_VALUE VALUES=" + valstr );
          readInputLine( getShortcutLabel() + "_var: CUSTOM ARG=" + getShortcutLabel() + "_sigma FUNC=x*x PERIODIC=NO");
       } else error("sigma has wrong length");
  } 

  // Create the reference point and arguments
  std::string refpoint, argstr; 
  for(unsigned i=0; i<argnames.size(); ++i) {  
      if( i==0 ) { argstr = argnames[0]; refpoint = getShortcutLabel() + "_" + fixArgumentDot(argnames[i]) + "_ref"; } 
      else { argstr += "," + argnames[1]; refpoint += "," + getShortcutLabel() + "_" + fixArgumentDot(argnames[i]) + "_ref"; }
  }
  ...
  unsigned nvals = argnames.size(); bool norm; parseFlag("NORMALIZED",norm);
  if( !usemahalanobis ) { 
      // Invert the variance
      ...
      if( norm ) {
          if( ktype=="von-misses" ) readInputLine( getShortcutLabel() + "_vec: CUSTOM ARG=" + getShortcutLabel() + "_icov FUNC=x PERIODIC=NO" );
          else readInputLine( getShortcutLabel() + "_det: PRODUCT ARG=" + getShortcutLabel() + "_var");
      }
  } else {
  ...
  std::string wstr; parse("WEIGHT",wstr);
  if( norm ) {
    if( ktype=="gaussian" ) {
