...
void Combine::read( ActionWithArguments* action ) {
  ...
  if(coefficients.size()!=nargs) action->error("Size of COEFFICIENTS array should be the same as number for arguments");
  parseVector(action,"PARAMETERS",parameters);
  if(parameters.size()!=nargs) action->error("Size of PARAMETERS array should be the same as number for arguments");
  parseVector(action,"POWERS",powers); if(powers.size()!=nargs) action->error("Size of POWERS array should be the same as number for arguments");
