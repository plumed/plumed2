...
{ 
  ...
  if( von_miss ) {
      ...
      readInputLine( getShortcutLabel() + "_scaled: CUSTOM ARG1=" + getShortcutLabel() + "_diffT FUNC=2*pi*x/(" + max +"-" + min + ") PERIODIC=NO");
      // We start calculating off-diagonal elements by computing the sines of the scaled differences (this is a column vector)
      readInputLine( getShortcutLabel() + "_sinediffT: CUSTOM ARG1=" + getShortcutLabel() + "_scaled FUNC=sin(x) PERIODIC=NO");
      ...
      readInputLine( getShortcutLabel() + "_prod: CUSTOM ARG1=" + getShortcutLabel() + "_scaled ARG2=" + metstr2 + " FUNC=2*(1-cos(x))*y PERIODIC=NO");
      ...
      readInputLine( getShortcutLabel() + "_diag: DOT DIAGONAL_ELEMENTS_ONLY ARG1=" + getShortcutLabel() + "_matones ARG2=" + getShortcutLabel() + "_prod");
      // Sum everything
