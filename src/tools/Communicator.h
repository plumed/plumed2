...
/// Init from pointer to VectorGeneric
    template <typename T, unsigned n> explicit Data(VectorGeneric<T,n> *p,int s): pointer(p), size(n*s), nbytes(sizeof(T)), type(getMPIType<T>()) {}
/// Init from reference to VectorGeneric
    template <typename T, unsigned n> explicit Data(VectorGeneric<T,n> &p): pointer(&p), size(n), nbytes(sizeof(T)), type(getMPIType<T>()) {}
/// Init from pointer to TensorGeneric
    template <typename T, unsigned n,unsigned m> explicit Data(TensorGeneric<T,n,m> *p,int s): pointer(p), size(n*m*s), nbytes(sizeof(T)), type(getMPIType<T>()) {}
/// Init from reference to TensorGeneric
    template <typename T, unsigned n,unsigned m> explicit Data(TensorGeneric<T,n,m> &p): pointer(&p), size(n*m), nbytes(sizeof(T)), type(getMPIType<T>()) {}
...
/// See Communicator::Data documentation
  struct ConstData {
    ...
    template <typename T,unsigned n> explicit ConstData(const VectorGeneric<T,n> *p,int s): pointer(p), size(n*s), nbytes(sizeof(T)), type(getMPIType<T>()) {}
    template <typename T,unsigned n> explicit ConstData(const VectorGeneric<T,n> &p): pointer(&p), size(n), nbytes(sizeof(T)), type(getMPIType<T>()) {}
    template <typename T, unsigned n,unsigned m> explicit ConstData(const TensorGeneric<T,n,m> *p,int s): pointer(p), size(n*m*s), nbytes(sizeof(T)), type(getMPIType<T>()) {}
    template <typename T, unsigned n,unsigned m> explicit ConstData(const TensorGeneric<T,n,m> &p): pointer(&p), size(n*m), nbytes(sizeof(T)), type(getMPIType<T>()) {}
