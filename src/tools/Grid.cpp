...
void Grid::erodeFunction(const double threshold_value, const vector<double> &smoothing_der) {
  ...
  while (is_not_smoothed) {
    ...
    if (point_value_assignment_options.size() == 0) {
      break;
    }
  ...
  for (unsigned i = 0; i < args.size(); ++i) {
    ...
    if (args[i]->isPeriodic()) {
      ...
      if (pmin != gmin[i] || pmax != gmax[i]) {
        std::string error_msg = "mismatch between grid boundaries (" + gmin[i] + ", " + gmax[i] + ") and periods of values (" + pmin + ", " + pmax + ")";
