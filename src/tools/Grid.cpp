...
// And read the stuff from the header
    ...
    if(!dosparse) {
        grid.reset(new Grid(funcl,args,gmin,gmax,gbin,dospline,doder));
    }
    else {
        grid.reset(new SparseGrid(funcl,args,gmin,gmax,gbin,dospline,doder));
    }
    ...
    for(unsigned i=0; i<maxsize_; ++i) {
        ...
        for(unsigned j=0; j<dimension_; ++j) {
            ...
            if( val1*val2<0 ) {
                ...
                for(unsigned k=0; k<dimension_; ++k) points[npoints][k]=getPoint(i)[k];
...
void SparseGrid::writeToFile(OFile& ofile) {
    ...
    for(const auto & it : map_) {
        ...
        ofile.printField(funcname, f);
        if(usederiv_) {
            for(unsigned j=0; j<dimension_; ++j) ofile.printField("der_" + argnames[j],der[j]);
        }
        ofile.printField();
    }
}
...
double Grid::integrate( std::vector<unsigned>& npoints ) {
    ...
    for(unsigned i=0; i<ntotgrid; ++i) {
        ...
        for(unsigned j=1; j<dimension_-1; ++j) {
            kk=(kk-t_index[j-1])/npoints[i-1];
            t_index[j]=(kk%npoints[i]);
        }
...
void Grid::mpiSumValuesAndDerivatives( Communicator& comm ) {
    comm.Sum( grid_ );
    for(unsigned i=0; i<der_.size(); ++i) comm.Sum( der_[i] );
}
