...
public:
    ...
    inline void setFromVector( const std::vector<T>& vecin ) {
        ...
        for(unsigned i=0; i<sz; ++i) data[i]=vecin[i];
    }
    ...
    unsigned isSymmetric() const {
        ...
        unsigned sym=1;
        for(unsigned i=1; i<rw; ++i) for(unsigned j=0; j<i; ++j) if( std::fabs(data[i+j*cl]-data[j+i*cl])>1.e-10 ) {
                    sym=0;
                    break;
                }
        return sym;
...
template <typename T> int Invert( const Matrix<T>& A, Matrix<double>& inverse ) {
    ...
    if( A.isSymmetric()==1 ) {
        ...
        k=0;
        for(unsigned i=0; i<A.rw; ++i) for(unsigned j=0; j<A.cl; ++j) inverse(j,i)=da[k++];
    }
...
template <typename T> void cholesky( const Matrix<T>& A, Matrix<T>& B ) {
    ...
    B=0.;
    for(unsigned i=0; i<A.rw; ++i) for(unsigned j=0; j<=i; ++j) B(i,j)+=L(i,j)*D[j];
}
