...
class NeighborList  
{
  ...
  const PLMD::Pbc& pbc_;
  vector<PLMD::AtomNumber> fullatomlist_,requestlist_;
  vector< pair<unsigned,unsigned> > neighbors_;
  double distance_;
...
/// of the two atoms forming the i-th pair among all possible pairs  
  pair<unsigned,unsigned> getIndexPair(unsigned i);
/// Extract the list of atoms from the current list of close pairs
  void setRequestList();
public:
  NeighborList(const vector<PLMD::AtomNumber>& list0,
               const vector<PLMD::AtomNumber>& list1,
               const bool& do_pair, const bool& do_pbc, const PLMD::Pbc& pbc, 
               const double& distance=1.0e+30, const unsigned& stride=0);
  NeighborList(const vector<PLMD::AtomNumber>& list0, const bool& do_pbc,
...
/// Return the list of all atoms. These are needed to rebuild the neighbor list.                         
  vector<PLMD::AtomNumber>& getFullAtomList();
...
/// Get the i-th pair of the neighbor list
  pair<unsigned,unsigned> getClosePair(unsigned i) const;
/// Get the list of neighbors of the i-th atom
  vector<unsigned> getNeighbors(unsigned i);
  ~NeighborList(){};
};
