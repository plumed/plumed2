...
//+PLUMEDOC COLVAR PIV       
/*
Calculates the PIV-distance: the squared Cartesian distance between the PIV \cite GalletPietrucci 
...
When NLIST and SFACTOR are used, values for each block have to be specified. 
...
In this second example we compute PIV-distances using Centers Of Mass (COM) of group of atoms instead of atom positions. COM Groups definition uses PDB residues columns, atoms belong to the same COM if they have both the same residuename and the same residuenumber. The residuename must be specified using the ATOMTYPES keyword. 
The atom mass must be specified in the occupancy column of the pdb file. In the following example four COMs are defined, two containing the oxygen atoms (atom indexes 1 and 5) and two containing the hydrogen atoms (atom indexes 2,3 and 6,7) 

PDB file:
\verbatim
ATOM      1  OW1 oxy     1      15.630  19.750   1.520  1.00 15.99
ATOM      2  HW2 hyd     1      15.480  20.660   1.240  1.00  0.00
ATOM      3  HW3 hyd     1      16.110  19.360   0.790  1.00  0.00
ATOM      5  OW1 oxy     2      20.490  18.680   1.520  1.00 15.99
ATOM      6  HW2 hyd     2      20.160  19.100   0.730  1.00  0.00
ATOM      7  HW3 hyd     2      21.290  19.150   1.730  1.00  0.00
\endverbatim
...
p1: FUNCPATHMSD ARG=Pivd1,Pivd2 LAMBDA=0.180338
METAD ARG=p1.s,p1.z SIGMA=0.01,0.2 HEIGHT=0.8 PACE=1000   LABEL=res
PRINT ARG=Pivd1,Pivd2,p1.s,p1.z,res.bias STRIDE=500  FILE=colvar FMT=%15.6f
...
When using such CV please cite \cite Pipolo_etal:arxive.
With the VOLUME keyword one scales the atom-atom distances by the cubic root of the ratio between the specified value and the box volume. 
...
class PIV      : public Colvar {   
  private:
    bool pbc, docomp, dosum, docmdist, serial;
    int Fdump,updatePIV;
...
void PIV::registerKeywords( Keywords& keys ){
  ...
  keys.add("optional","DUMPTIME","The frequency (timesteps) to dump info on performances.");
...
//com2atoms(std:: vector<std:: vector<unsigned> >(Nlist))
{
  ...
  pbc=!nopbc;  
  if(pbc) {log << "Using Periodic Boundary Conditions\n";
  } else  {log << "Isolated System (NO PBC)\n";}   
  
  // SERIAL/PARALLEL
  parseFlag("SERIAL",serial);
  if(serial) {log << "Serial PIV construction\n";
  ...
  // Timing     
  if(keywords.exists("DUMPTIME")){parse("DUMPTIME",Fdump);}

  // Test
  parseFlag("TEST",test);

  // UPDATEPIV
  if(keywords.exists("UPDATEPIV")){parse("UPDATEPIV",updatePIV);}
  ...
  if (oc&&od) {error("ONLYCROSS and ONLYDIRECT are incompatible options!");}
  if(oc){direct=false; log << "Using only CROSS-PIV blocks\n";}
  if(od){ cross=false; log << "Using only DIRECT-PIV blocks\n";}
  ...
  for (unsigned i=0;i<mypdb.getAtomNumbers().size();i++) {
    ...
    bool oldres=false;
    for (unsigned j=0;j<Presind.size();j++) {if(rind==Presind[j]) {oldres=true;}}
    if(!oldres){Presind.push_back(rind);}
  }
  resnum=Presind.size();

  // Pind0 is the atom/COM used in Nlists (for COM Pind0 is the first atom in the pdb belonging to that COM)
  unsigned Pind0size;
  if(com){Pind0size=resnum;
  } else {Pind0size=NLsize;}
  ...
  for (unsigned j=0;j<Natm;j++) {
    unsigned oind;
    for (unsigned i=0;i<Pind0.size();i++) {Pind0[i]=0;}
    for (unsigned i=0;i<mypdb.getAtomNumbers().size();i++) {
      // Residue/Atom AtomNumber: used to build NL for COMS/Atoms pairs. 
      //AtomNumber rnum=mypdb.getResidueNumbers()[i];
      ...
      unsigned aind=mypdb.getAtomIndex()[i];
      // This builds lists for NL
      string Pname;
      unsigned Pind;
      if(com){Pname=rname;for(unsigned l=0;l<resnum;l++){if(rind==Presind[l]){Pind=l;}}
      } else {Pname=aname;Pind=aind-1;}
      if(Pname==atype[j]) {
        if(Pind0[Pind]==0) {
  ...
  for (unsigned i=0;i<mypdb.getAtomNumbers().size();i++) {listall.push_back(mypdb.getAtomNumbers()[i]);}

  // PIV blocks and Neighbour Lists
  Nlist=0;
  // Direct adds the A-A ad B-B blocks (N)
  if(direct) {Nlist=Nlist+unsigned(Natm);}
  // Cross adds the A-B blocks (N*(N-1)/2)
  if(cross) {Nlist=Nlist+unsigned(double(Natm*(Natm-1))/2.);}
  // Resize vectors according to Nlist
  rPIV.resize(Nlist);
    
  // PIV scaled option
  scaling.resize(Nlist);
  for(unsigned j=0;j<Nlist;j++) {scaling[j]=1.;}
  if(keywords.exists("SFACTOR")){
    log << "Reading Scaling Factor parameters \n";
  ...
  if(doneigh){
    ...
    log << "Reading Neighbor Lists parameters \n";
    ...
    for (unsigned j=0;j<Nlist;j++) {nl[j]= new NeighborList(Plist[j],Plist[j],true,pbc,getPbc());}
  }
  // Output Nlist
  ...
  if(com){
    ...
    for(unsigned j=0;j<compos.size();j++) {
      ...
      for(unsigned i=0;i<nlcom[j]->getFullAtomList().size();i++){
        unsigned andx=nlcom[j]->getFullAtomList()[i].index();
        if(commass>0.){fmass[andx]=mypdb.getOccupancy()[andx]/commass;
        } else {       fmass[andx]=1.;}  
      }
    }
  }
  // Read Parameters and set-up switching functions
  log << "Reading switching functions  \n";
  sfs.resize(Nlist);
  std::string errors;
  for (unsigned j=0;j<Nlist;j++) {
    std::string num, sw; Tools::convert(j+1, num);
  ...
  for (unsigned i=0;i<Nlist;i++) {
    if(ynsort[i]==0||CompDer){dosort[i]=false;}else{dosort[i]=true;}
  }
  ...
  } else {log << "Using unscaled atom distances \n";}
  ...
  //Set switching function parameters: now set at the beginning of the dynamics to solve the r0 issue
    ...
    for (unsigned j=0;j<Nlist;j++) {
      ...
      double d0=sfs[j].get_d0(); 
      if(Svol) {r0*=Fvol;} 
      r00[j]=r0;    
      sfs[j].set(nn,mm,r0,d0);
      log << "  Swf: " << j << "   d0: " <<  d0 << "   r0: " << r0 << "   nn: " << nn << "   mm: " << mm << " \n";
    }
  }
  ...
  if(com){
    for(unsigned j=0;j<compos.size();j++) {
      compos[j][0]=0.;compos[j][1]=0.;compos[j][2]=0.;
      for(unsigned i=0;i<nlcom[j]->getFullAtomList().size();i++){
        unsigned andx=nlcom[j]->getFullAtomList()[i].index();
  ...
  if(CompDer) {log << "  PIV  |  block   |     Size      |     Zeros     |     Ones      |" << " \n";}
  for(unsigned j=0;j<Nlist;j++) {
    for(unsigned i=0;i<nl[j]->size();i++) {
      unsigned i0=nl[j]->getOIndexPair(i).first;
      unsigned i1=nl[j]->getOIndexPair(i).second;
      ...
      Vector ddist;
      if(pbc){ddist=mypbc.distance(Pos0,Pos1);
      } else {  ddist=delta(Pos0,Pos1);}
      double df=0.;
      // Transformation and sorting done at the first timestep to solve the r0 definition issue
      if(CompDer){rPIV[j].push_back(sfs[j].calculate(ddist.modulo()*Fvol, df));
      } else {    rPIV[j].push_back(ddist.modulo()*Fvol);}
    }
    if(CompDer){
      if(dosort[j]){std::sort(rPIV[j].begin(),rPIV[j].end());}
      int lmt0=0;
      int lmt1=0;
      for(unsigned i=0;i<rPIV[j].size();i++){
        if(int(rPIV[j][i]*double(Nprec-1))==0) {lmt0+=1;} 
        if(int(rPIV[j][i]*double(Nprec-1))==1) {lmt1+=1;} 
      } 
...
PIV::~PIV(){
  for (unsigned j=0;j<Nlist;j++) {delete nl[j];}
  if(com) {for (unsigned j=0;j<NLsize;j++) {delete nlcom[j];}}
  delete nlall;
}
...
void PIV::calculate(){ 
  ...
  static std:: vector<std:: vector<int> > A0(Nprec);
  static std:: vector<std:: vector<int> > A1(Nprec);
  ...
  if ((prev_stp==-1||(prev_stp==0&&getStep()==0))&&!CompDer) {
    ...
    if(Svol) {Fvol=cbrt(Vol0/getBox().determinant());} 
  ...
  if ((getStep()>prev_stp&&getStep()%updatePIV==0)||CompDer) {
    ...
    if (doneigh) {
      ...
      if (prev_stp==-1) {
        ...
        for (unsigned j=0;j<Nlist;j++) {
          for (unsigned i=0;i<nl[j]->getFullAtomList().size();i++) {
            Vector Pos;
            if(docom){Pos=compos[i];
            } else {Pos=getPosition(nl[j]->getFullAtomList()[i].index());}
      ...
      if (getStep() % nlall->getStride() ==0) {
        ...
        for (unsigned j=0;j<Nlist;j++) {
          for (unsigned i=0;i<nl[j]->getFullAtomList().size();i++) {
            ...
            if (pbcDistance(tmp_pos[j][i],prev_pos[j][i]).modulo()>=nl_skin[j]) {doupdate=true;}
          }
        }
      }
      ...
      if (doupdate==true) {
        for (unsigned j=0;j<Nlist;j++) {
          for (unsigned i=0;i<nl[j]->getFullAtomList().size();i++) {prev_pos[j][i]=tmp_pos[j][i];}
    ...
    for(unsigned j=0;j<Nlist;j++) {
      if(dosort[j]) {      
        ...
        Atom0[j].resize(0);
        Atom1[j].resize(0);
        // Update switching function if required: DEPRECATED
        //if(Svol&&updatesfs) {
        //  int nn=sfs[j].get_nn(); 
        //  int mm=sfs[j].get_mm();
        //  double r0=r00[j]*Fvol; 
        //  double d0=sfs[j].get_d0(); 
        //  sfs[j].set(nn,mm,r0,d0);
        //}
        // Building distances for the PIV vector at time t
	for(unsigned i=rank;i<nl[j]->size();i+=stride) {
          unsigned i0=nl[j]->getOClosePair(i).first;
          unsigned i1=nl[j]->getOClosePair(i).second;
          Vector Pos0,Pos1;
          if(docom){
            Pos0=compos[i0]; 
            Pos1=compos[i1]; 
          ...
          if(dopbc){ddist=pbcDistance(Pos0,Pos1);
          } else {  ddist=delta(Pos0,Pos1);}
          double df=0.;
	  ...
	  //Integer transformed distance values as index of the Ordering Vector OrdVec          
        ...
        if(!doserial) {
          ...
          //Timing
          //t1 += difftime( time(0), start);
          //if((getStep()+1) % Fdump == 0) {log << "Loop1: " << t1 << " sec\n";
          //                              log << "Stride: " << stride << " \n";}
          //start = time(0);
          ...
          //if((getStep()+1) % Fdump == 0) {log << "Loop2: " << t2 << " sec\n";}
          //start = time(0);
    ...
    if((getStep()+1) % Fdump == 0) {log << "Loop-cur: " << t3 << " sec\n";}
  }
  ...
  if(test) {
    unsigned limit=0;
    for(unsigned j=0;j<Nlist;j++) { 
      if(dosort[j]) {limit = cPIV[j].size();
      } else {       limit = rPIV[j].size();}
      ...
      for(unsigned i=0;i<limit;i++) { 
        ...
        if(dosort[j]){
          ...
          i0=nl[j]->getOClosePair(i).first;
          i1=nl[j]->getOClosePair(i).second;
        }
        Vector Pos0,Pos1;
        if(com){
          Pos0=compos[i0]; 
        ...
        if(pbc){distance=pbcDistance(Pos0,Pos1);
        } else {distance=delta(Pos0,Pos1);}
        dfunc=0.;
        double cP,rP;
        if(dosort[j]) {
          cP = cPIV[j][i];
  ...
  Tensor virial;
  for(unsigned j=0;j<3;j++){for(unsigned k=0;k<3;k++){virial[j][k]=0.;}}
  ...
  if(getStep()%updatePIV==0) {
    ...
    for(unsigned j=0;j<Nlist;j++) {
      ...
      bool docom=com;
      bool dopbc=pbc;
      if(dosorting) {limit = cPIV[j].size();
      } else {       limit = rPIV[j].size();}
      for(unsigned i=rank;i<limit;i+=stride) {
        ...
        if(dopbc){distance=pbcDistance(Pos0,Pos1);
        } else {  distance=delta(Pos0,Pos1);}
        dfunc=0.;
        ...
        // PIV distance
        double coord=0.;
        if(!dosorting||Nder) {coord = tPIV - rPIV[j][i];
        } else {              coord = cPIV[j][i] - rPIV[j][rPIV[j].size()-cPIV[j].size()+i];}
        ...
        if(docom){
          ...
          for(unsigned k=0;k<nlcom[i0]->getFullAtomList().size();k++){
            ...
            for(unsigned l=0;l<3;l++){dist[l]=0.;}
            ...
            for(unsigned l=0;l<nlcom[i0]->getFullAtomList().size();l++){
              ...
              if(dopbc){dist+=pbcDistance(P0,P1);
              } else {  dist+=delta(P0,P1);}
            }
          ...
          for(unsigned k=0;k<nlcom[i1]->getFullAtomList().size();k++){
            ...
            for(unsigned l=0;l<nlcom[i1]->getFullAtomList().size();l++){
              ...
              Vector P0=getPosition(x0);
              if(dopbc){dist+=pbcDistance(P1,P0);
              } else {  dist+=delta(P1,P0);}
            }
            for(unsigned l=0;l<nlcom[i0]->getFullAtomList().size();l++){
        ...
        if(Scalevol) {virial+=1./3.*tmp*dm*dm*Tensor::identity();}
    ...
    if (getStep()>prev_stp) {
      for(unsigned j=0;j<getNumberOfAtoms();j++) {for(unsigned i=0;i<3;i++) {deriv2[j][i]=deriv[j][i];}};
      for(unsigned j=0;j<3;j++) {for(unsigned i=0;i<3;i++) {virial2[j][i]=virial[j][i];}};
      PIVdistance2=PIVdistance;
    } else {
      for(unsigned j=0;j<getNumberOfAtoms();j++) {for(unsigned i=0;i<3;i++) {deriv1[j][i]=deriv[j][i];}};
      for(unsigned j=0;j<3;j++) {for(unsigned i=0;i<3;i++) {virial1[j][i]=virial[j][i];}};
      PIVdistance1=PIVdistance;
    }
  } else {
    if (getStep()>prev_stp) {
      for(unsigned j=0;j<getNumberOfAtoms();j++) {for(unsigned i=0;i<3;i++) {deriv[j][i]=deriv2[j][i];}};
      for(unsigned j=0;j<3;j++) {for(unsigned i=0;i<3;i++) {virial[j][i]=virial2[j][i];}};
      PIVdistance=PIVdistance2;
    } else {
      for(unsigned j=0;j<getNumberOfAtoms();j++) {for(unsigned i=0;i<3;i++) {deriv[j][i]=deriv1[j][i];}};
      for(unsigned j=0;j<3;j++) {for(unsigned i=0;i<3;i++) {virial[j][i]=virial1[j][i];}};
  ...
  if((getStep()+1) % Fdump == 0) {log << "Loop-ref: " << t4 << " sec\n";}
