...
Mapping::Mapping(const ActionOptions&ao):
  ...
  if( keywords.exists("PROPERTY") ) {
    ...
    property.insert( std::pair<std::string,std::vector<double> >( "spath", std::vector<double>() ) );
    ispath=true;
  }
...
void Mapping::finishPackSetup( const unsigned& ifunc, ReferenceValuePack& mypack ) const {
  ...
  if( nat2>0 ) {
    ReferenceAtoms* myat2=dynamic_cast<ReferenceAtoms*>( myframes[ifunc].get() );
    plumed_dbg_assert( myat2 );
    for(unsigned i=0; i<nat2; ++i) {
      mypack.setAtomIndex( i, myat2->getAtomIndex(i) );
    }
  }
}
...
double Mapping::calculateDistanceFunction( const unsigned& ifunc, ReferenceValuePack& myder, const bool& squared ) const {
  ...
  if( getNumberOfAtoms()>0 && !myder.virialWasSet() ) {
    Tensor tvir;
    tvir.zero();
    for(unsigned i=0; i<myder.getNumberOfAtoms(); ++i) {
      tvir +=-1.0*Tensor( getPosition( myder.getAtomIndex(i) ), myder.getAtomDerivative(i) );
    }
