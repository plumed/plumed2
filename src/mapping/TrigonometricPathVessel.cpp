    ...
    mypack3( 0, 0, mydpack3 )
{
    ...
    std::vector<std::string> argument_names( mymap->getNumberOfArguments() );
...
void TrigonometricPathVessel::finish( const std::vector<double>& buffer ) {
    ...
    if( mindist2<mindist1 ) {
        ...
        mindist1=mindist2;
        mindist2=tmp;
        iclose1=getStoreIndex(1);
        iclose2=getStoreIndex(0);
    }
    ...
    if( mymap->getNumberOfAtoms()>0 ) {
        for(unsigned i=0; i<mymap->getNumberOfAtoms(); ++i) {
            Vector dxder;
            for(unsigned k=0; k<3; ++k) dxder[k] = ( 2*v4v4*dx - 2*proj )*spacing*sp->getDerivative( narg + 3*i+k );
