...
bool MultiColvarBase::parseMultiColvarAtomList(const std::string& key, const int& num, std::vector<AtomNumber>& t) {
  ...
  for(unsigned i=0; i<mlabs.size(); ++i) {
    ...
    for(unsigned j=0; j<mycolv->getFullNumberOfTasks(); ++j) {
      atom_lab.push_back( std::pair<unsigned,unsigned>( mybasemulticolvars.size()+1, j ) );
    }
  ...
  if( found_mcolv==0 ) {
    std::vector<AtomNumber> tt;
    ActionAtomistic::interpretAtomList( mlabs, tt );
    for(unsigned i=0; i<tt.size(); ++i) {
      atom_lab.push_back( std::pair<unsigned,unsigned>( 0, t.size() + i ) );
    }
    log.printf("  keyword %s takes atoms : ", key.c_str() );
    for(unsigned i=0; i<tt.size(); ++i) {
      t.push_back( tt[i] );
      log.printf("%d ",tt[i].serial() );
    }
    log.printf("\n");
  } else if( found_mcolv==mlabs.size() ) {
    if( checkNumericalDerivatives() ) {
      error("cannot use NUMERICAL_DERIVATIVES keyword with dynamic groups as input");
    }
...
void MultiColvarBase::readTwoGroups( const std::string& key0, const std::string& key1, const std::string& key2, std::vector<AtomNumber>& all_atoms ) {
  ...
  if( parseMultiColvarAtomList(key0,-1,all_atoms) ) {
    ...
    ablocks[1].resize( atom_lab.size() - ablocks[0].size() );
    for(unsigned i=0; i<ablocks[1].size(); ++i) {
      ablocks[1][i]=ablocks[0].size() + i;
    }
...
void MultiColvarBase::readGroupKeywords( const std::string& key0, const std::string& key1, const std::string& key2, const std::string& key3,
  ...
  if( parseMultiColvarAtomList(key0,-1,all_atoms) ) {
    if( no_third_dim_accum ) {
      nblock=atom_lab.size();
      ablocks[0].resize(nblock);
      ablocks[1].resize( nblock );
      for(unsigned i=0; i<ablocks[0].size(); ++i) {
        ablocks[0][i]=ablocks[1][i]=i;
      }
      resizeBookeepingArray( nblock, nblock );
      if( symmetric ) {
        ...
        for(unsigned i=0; i<nblock; ++i) {
          bookeeping(i,i).first=0;
          bookeeping(i,i).second=1;
        }
        for(unsigned i=1; i<nblock; ++i) {
          for(unsigned j=0; j<i; ++j) {
      ...
      if( !parseMultiColvarAtomList(key3,-1,all_atoms) ) {
        error("missing required keyword " + key3 + " in input");
      }
      ablocks[2].resize( atom_lab.size() - ablocks[0].size() );
      for(unsigned i=0; i<ablocks[2].size(); ++i) {
        ablocks[2][i]=ablocks[0].size() + i;
      }
    } else {
      nblock=atom_lab.size();
        ...
        for(unsigned i=2; i<nblock; ++i) {
          for(unsigned j=1; j<i; ++j) {
            ...
            for(unsigned k=0; k<j; ++k) {
              addTaskToList( i*nblock*nblock + j*nblock + k );
            }
            bookeeping(i,j).second=getFullNumberOfTasks();
          }
        }
...
void MultiColvarBase::readThreeGroups( const std::string& key1, const std::string& key2, const std::string& key3,
  ...
  if( !readkey3 && !allow2 ) {
    ...
    for(unsigned i=0; i<ablocks[2].size(); ++i) {
      ablocks[2][i] = ablocks[0].size() + ablocks[1].size() + i;
    }
...
void MultiColvarBase::resizeBookeepingArray( const unsigned& num1, const unsigned& num2 ) {
  ...
  for(unsigned i=0; i<num1; ++i) {
    for(unsigned j=0; j<num2; ++j) {
      bookeeping(i,j).first=0;
      bookeeping(i,j).second=0;
    }
  }
}
...
void MultiColvarBase::setupMultiColvarBase( const std::vector<AtomNumber>& atoms ) {
  ...
  if( !usespecies && nblock>0 ) {
    ...
    if( allthirdblockintasks ) {
      ...
      if( std::pow( double(nblock), 2.0 )>std::numeric_limits<unsigned>::max() ) {
        error("number of atoms in groups is too big for PLUMED to handle");
      }
    } else {
      decoder.resize( ablocks.size() );
      // Check if number of atoms is too large
      if( std::pow( double(nblock), double(ablocks.size()) )>std::numeric_limits<unsigned>::max() ) {
        error("number of atoms in groups is too big for PLUMED to handle");
      }
    }
    unsigned code=1;
    for(unsigned i=0; i<decoder.size(); ++i) {
      decoder[decoder.size()-1-i]=code;
      code *= nblock;
    }
  } else if( !usespecies ) {
    ncentral=ablocks.size();
    ...
    if( readspecies ) {
      ...
      if( !parseMultiColvarAtomList("SPECIESA", -1, all_atoms) ) {
        error("missing SPECIES/SPECIESA keyword");
      }
      unsigned nat1=atom_lab.size();
      if( !parseMultiColvarAtomList("SPECIESB", -1, all_atoms) ) {
        error("missing SPECIESB keyword");
      }
      unsigned nat2=atom_lab.size() - nat1;

      for(unsigned i=0; i<nat1; ++i) {
        addTaskToList(i);
      }
  ...
  for(unsigned i=0; i<mybasemulticolvars.size(); ++i) {
    addDependency( mybasemulticolvars[i] );
  }
...
void MultiColvarBase::setupLinkCells() {
  ...
  if( nactive_atoms>0 ) {
    ...
    if( usespecies ) {
      ...
      for(unsigned i=0; i<ablocks[1].size(); ++i) {
        if( !isCurrentlyActive( ablocks[1][i] ) ) {
          continue;
        }
...
void MultiColvarBase::setupNonUseSpeciesLinkCells( const unsigned& my_always_active ) {
  ...
  if( !uselinkforthree && nactive_atoms>0 ) {
    ...
    for(unsigned i=rank; i<ablocks[0].size(); i+=stride) {
      ...
      unsigned natomsper=1;
      linked_atoms[0]=my_always_active;  // Note we always check atom 0 because it is simpler than changing LinkCells.cpp
      ...
      for(unsigned j=0; j<natomsper; ++j) {
        for(unsigned k=bookeeping(i,linked_atoms[j]).first; k<bookeeping(i,linked_atoms[j]).second; ++k) {
          taskFlags[k]=1;
        }
      }
    }
    ...
    for(unsigned i=0; i<ablocks[2].size(); ++i) {
      if( isCurrentlyActive( ablocks[2][i] ) ) {
        nactive_three++;
      }
    }
    ...
    for(unsigned i=rank; i<ablocks[0].size(); i+=stride) {
      ...
      if( allthirdblockintasks ) {
        ...
        unsigned ntatomsper=1;
        tlinked_atoms[0]=lttmp_ind[0];
        threecells.retrieveNeighboringAtoms( getPositionOfAtomForLinkCells( ablocks[0][i] ), requiredlinkcells, ntatomsper, tlinked_atoms );
        for(unsigned j=0; j<natomsper; ++j) {
          for(unsigned k=0; k<ntatomsper; ++k) {
            taskFlags[bookeeping(i,linked_atoms[j]).first+tlinked_atoms[k]]=1;
          }
        }
      }
    }
...
bool MultiColvarBase::setupCurrentAtomList( const unsigned& taskCode, AtomValuePack& myatoms ) const {
  if( isDensity() ) {
    myatoms.setNumberOfAtoms( 1 );
    myatoms.setAtom( 0, taskCode );
    return true;
  } else if( usespecies ) {
    std::vector<unsigned> task_atoms(1);
    ...
    for(unsigned i=0; i<ablocks.size(); ++i) {
      myatoms.setAtom( i, atoms[i] );
    }
  } else {
    myatoms.setNumberOfAtoms( ablocks.size() );
    for(unsigned i=0; i<ablocks.size(); ++i) {
      myatoms.setAtom( i, ablocks[i][taskCode] );
    }
  }
  return true;
}
  ...
  if( getLabel()!=input_label ) {
    ...
    for(unsigned i=0; i<getFullNumberOfTasks(); ++i) {
      ...
      for(unsigned j=0; j<mytmp_atoms.getNumberOfAtoms(); ++j) {
        ...
        if( atom_lab[itask].first==input_code ) {
          active_tasks[ atom_lab[itask].second ]=1;
        }
      }
    }
  }
...
void MultiColvarBase::splitInputDerivatives( const unsigned& ival, const unsigned& start, const unsigned& end,
  ...
  for(unsigned j=0; j<myder.getNumberActive(); ++j) {
    ...
    if( jder<mybasemulticolvars[mmc]->getNumberOfDerivatives()-9 ) {
      unsigned kder=basen+jder;
      plumed_assert( kder<myvals.getNumberOfDerivatives() );
      for(unsigned icomp=start; icomp<end; ++icomp) {
...
void MultiColvarBase::addComDerivatives( const int& ival, const unsigned& iatom, const Vector& der, multicolvar::AtomValuePack& myatoms ) const {
  ...
  unsigned basen=0;
  for(unsigned i=0; i<mmc; ++i) {
    basen+=(mybasemulticolvars[i]->getNumberOfDerivatives() - 9) / 3;
  }
...
void MultiColvarBase::performTask( const unsigned& task_index, const unsigned& current, MultiValue& myvals ) const {
  ...
  if( !matsums ) {
    for(unsigned i=0; i<myatoms.getNumberOfAtoms(); ++i) {
      if( atom_lab[myatoms.getIndex(i)].first==0 ) {
        continue;
      }
    ...
    if( atom_lab[myatoms.getIndex(0)].first>0 ) {
      if( mybasedata[atom_lab[myatoms.getIndex(0)].first-1]->retrieveWeightWithIndex( atom_lab[myatoms.getIndex(0)].second )<epsilon ) {
        weight=0.;
      }
    }
  }
  ...
  if( !matsums && !doNotCalculateDerivatives() && mybasemulticolvars.size()>0 ) {
    ...
    for(unsigned i=0; i<myatoms.getNumberOfAtoms(); ++i) {
      ...
      for(unsigned j=0; j<myder.getNumberActive(); ++j) {
        unsigned jder=myder.getActiveIndex(j);
        outder.addDerivative( 0, jder, prefactor*myder.getDerivative(0,jder) );
      }
      myder.clearAll();
    }
  }
...
Vector MultiColvarBase::getCentralAtomPos( const unsigned& taskIndex ) {
  ...
  if( usespecies || isDensity() ) {
    ...
    for(unsigned i=0; i<ablocks.size(); ++i) {
      if( use_for_central_atom[i] ) {
        mypos+=numberForCentralAtom*getPositionOfAtomForLinkCells(atoms[i]);
      }
    }
    return mypos;
  } else {
    ...
    for(unsigned i=0; i<ablocks.size(); ++i) {
      if( use_for_central_atom[i] ) {
        mypos+=numberForCentralAtom*getPositionOfAtomForLinkCells(ablocks[i][curr]);
      }
    }
    return mypos;
  }
...
void MultiColvarBase::getCentralAtomPack( const unsigned& basn, const unsigned& taskIndex, CatomPack& mypack ) {
  ...
  if(usespecies) {
    if( mypack.getNumberOfAtomsWithDerivatives()!=1 ) {
      mypack.resize(1);
    }
