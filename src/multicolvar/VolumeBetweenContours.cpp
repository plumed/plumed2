  ...
  mylinks(comm)
{
  ...
  kernel=std::make_unique<KernelFunctions>( pp, bandwidth, getKernelType(), "DIAGONAL", 1.0 );
  for(unsigned i=0; i<3; ++i) { pos.emplace_back(std::make_unique<Value>()); pos[i]->setNotPeriodic(); }
