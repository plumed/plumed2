...
DistanceFromContour::DistanceFromContour( const ActionOptions& ao ):
  ...
  for(unsigned i=0; i<mybasemulticolvars[0]->getFullNumberOfTasks(); ++i) {
    addTaskToList(i);
  }
  // And a cutoff
  ...
  for(unsigned i=0; i<3; ++i) {
    pval.emplace_back( Tools::make_unique<Value>() );
  }
}
...
void DistanceFromContour::calculate() {
  ...
  for(unsigned j=1; j<getNumberOfAtoms()-1; ++j) {
    ...
    if( (d2=distance[perp_dirs[0]]*distance[perp_dirs[0]])<rcut2 &&
      ...
      if( d2<mindist && std::fabs(distance[dir])>epsilon ) {
  ...
  if( !doNotCalculateDerivatives() ) {
    ...
    for(unsigned i=0; i<val->getNumberOfDerivatives(); ++i) {
      val->setDerivative( i, -prefactor*ival->getDerivative(i) );
    }
    ival->clearDerivatives();
    std::vector<double> root2v(3);
    for(unsigned i=0; i<3; ++i) {
      root2v[i]=root2[i];
    }
...
double DistanceFromContour::compute( const unsigned& tindex, AtomValuePack& myatoms ) const {
  ...
  std::vector<double> pp(3), der(3,0);
  for(unsigned j=0; j<3; ++j) {
    pp[j] = distance[j];
  }
