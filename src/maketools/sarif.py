...
# some manual extra rules from codecheck (when upgrading codecheck I would separate the sarif file that codecheck produces from this, to have better descriptions)
RULES["uninitvar"] = {
    "name": "uninitvar",
...
class Finding:
    ...
    def __post_init__(self):
        ...
        # most of the plumedcheck results are relative to the src directory
        # but gh wants a path relative to the base directory
        if self.file:
            # this covers plugins and other directories
            if self.file.startswith("../"):
                self.file = self.file.removeprefix("../")
            # ruff (the python checks) reports files referred the the base directory
            elif not (self.file.endswith(".py") or self.file.endswith(".pyx")):
                # this covers the src directory
                self.file = f"src/{self.file}"

        # GitHub does not accepts line=0 for "whole file" problems
        if self.line is not None and self.line == 0:
            self.line = 1

        # telling that the astyle problem concerns the whole file
        # If you want to generalize this use the substitute the previous `if self.line is not None and self.line == 0:` body with this, I am not sure it is a good idea DR
        if self.ruleId == "plmd_astyle" and self.file:
            f = self.file
            if f not in linesPerFile:
                # this runs in `${basedir}/src` and the filenames have been remapped on `${basedir}`
                with open("../" + f, "r") as fp:
...
def parse_line(line):
    ...
    if ":ruff_check:" in line:
        # ruff outputs its own sarif file
        return None
