...
/// Get the number of derivatives
  unsigned getNumberOfDerivatives() const override { return getNumberOfArguments(); }
/// Do the calculation
...
void ArgsToVatom::calculate() {
  if( fractional ) {
      if( pbc_action->getPbc().isSet() ) {
          ...
          // Set the values
          for(unsigned i=0;i<3;++i) getPntrToOutput(i)->set( getPntrToArgument(5+i)->get() );
          // And the derivatives
          for(unsigned i=0;i<3;++i) getPntrToOutput(i)->addDerivative( 5+i, 1.0 );
      }
      // Set the mass and charge
      for(unsigned i=3;i<5;++i) getPntrToOutput(i)->set( getPntrToArgument(i)->get() );
  } else {
      // Set the values
      for(unsigned i=0;i<5;++i) getPntrToOutput(i)->set( getPntrToArgument(i)->get() );
      // And the derivatives
      for(unsigned i=0;i<3;++i) getPntrToOutput(i)->addDerivative( i, 1.0 );
  }
}

void ArgsToVatom::apply() {
  std::vector<double> forcesToApply(getNumberOfArguments(), 0); unsigned start=0;
  if( getForcesFromValues( forcesToApply ) ) setForcesOnArguments( 0, forcesToApply, start );
}
