    ...
    output_for_all_replicas(false)
{
    ...
    if( getArguments().size()==0 ) {
        ...
        for(unsigned i=0; i<tmp_vals.size(); ++i) req_vals[i]=tmp_vals[i];
    } else {
        req_vals.resize( getArguments().size() );
        for(unsigned i=0; i<req_vals.size(); ++i) req_vals[i]=getPntrToArgument(i)->getName();
    }
    if( req_vals.size()==0 ) {
    ...
    std::vector<std::string> rep_data;
    parseVector("REPLICA",rep_data);
    if( rep_data.size()==1 ) {
        if( rep_data[0]=="all" ) output_for_all_replicas=true;
        else {
            preps.resize(1);
            Tools::convert( rep_data[0], preps[0] );
        }
    } else {
