...
void ReadAnalysisFrames::registerKeywords( Keywords& keys ) {
  ...
  keys.remove("SERIAL");
  keys.remove("USE_OUTPUT_DATA_FROM");
  keys.use("ARG");
...
ReadAnalysisFrames::ReadAnalysisFrames( const ActionOptions& ao ):
  ...
  if( clearstride!=0 ) {
    log.printf("  clearing stored data every %u steps\n",clearstride);
  }
  ...
  if( atom_numbers.size()>0 ) {
    ...
    for(unsigned i=0; i<atom_numbers.size(); ++i) {
      log.printf("%d ",atom_numbers[i].serial() );
    }
    log.printf("\n");
  ...
  if( wwstr.size()>0 ) {
    ...
    if( wham_pointer && weight_vals.size()!=1 ) {
      error("can only extract weights from one wham object");
    }
  } else {
...
void ReadAnalysisFrames::update() {
  ...
  if( clearonnextstep ) {
    my_data_stash.clear();
    my_data_stash.resize(0);
    logweights.clear();
    logweights.resize(0);
    if( wham_pointer ) {
      wham_pointer->clearData();
    }
    clearonnextstep=false;
  }
  ...
  double ww=0;
  for(unsigned i=0; i<weight_vals.size(); ++i) {
    ww+=weight_vals[i]->get();
  }
  weights_calculated=false;
  logweights.push_back(ww);

  // Now create the data collection object and push it back to be stored
  unsigned index = my_data_stash.size();
  my_data_stash.push_back( DataCollectionObject() );
  ...
  for(unsigned i=0; i<argument_names.size(); ++i) {
    my_data_stash[index].setArgument( argument_names[i], getArgument(i) );
  }

  if( clearstride>0 ) {
    if( getStep()%clearstride==0 ) {
      clearonnextstep=true;
    }
  }
}
