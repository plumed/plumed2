...
void EuclideanDissimilarityMatrix::performAnalysis() {
  ...
  if( !usingLowMem() ) {
    dissimilarities.resize( getNumberOfDataPoints(), getNumberOfDataPoints() );
    dissimilarities=0;
  }
}
...
double EuclideanDissimilarityMatrix::getDissimilarity( const unsigned& iframe, const unsigned& jframe ) {
  ...
  if( !usingLowMem() ) {
    if( dissimilarities(iframe,jframe)>0. ) {
  ...
  if( iframe!=jframe ) {
    ...
    if( !usingLowMem() ) {
      dd=dissimilarities(iframe,jframe) = dissimilarities(jframe,iframe) = distance( getPbc(), getArguments(), myref1.get(), myref2.get(), true );
    } else {
      dd=distance( getPbc(), getArguments(), myref1.get(), myref2.get(), true );
    }
    return dd;
  }
  return 0.0;
