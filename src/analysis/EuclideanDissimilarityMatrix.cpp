...
void EuclideanDissimilarityMatrix::registerKeywords( Keywords& keys ) {
  ...
  keys.addFlag("LOWMEM",false,"lower the memory requirement");
}
...
EuclideanDissimilarityMatrix::EuclideanDissimilarityMatrix( const ActionOptions& ao ):
  ...
  AnalysisBase(ao)
{
  parse("METRIC",mtype); parseFlag("LOWMEM",lowmem); std::vector<AtomNumber> atoms;
...
double EuclideanDissimilarityMatrix::getDissimilarity( const unsigned& iframe, const unsigned& jframe ) {
  ...
  if( iframe!=jframe ) {
    ...
    if( !lowmem ) dd=dissimilarities(iframe,jframe) = dissimilarities(jframe,iframe) = distance( getPbc(), getArguments(), myref1.get(), myref2.get(), true );
