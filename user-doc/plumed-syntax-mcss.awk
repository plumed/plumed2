...
# DRAFT LINK TO DOC:
 ...
 if(length(action)>0){
   ...
   if(getline tmp < actionfile < 0) print "WARNING: file " actionfile " does not exist" > "/dev/stderr";
     ...
     p=match($0,action);
     $0=substr($0,1,p-1) "<a href=\"./" actionx ".html\" style=\"color:orange\">" action "</a>" substr($0,p+length(action));
   }
 }
...
#  if(match($0,"span style=")) $0=$0 "</span>";
  sub("#.*$","<span style=\"color:lightgreen\">&</span>");
  if(endplumed) sub("^.*$","<span style=\"color:lightgreen\">&</span>");
  print
}
