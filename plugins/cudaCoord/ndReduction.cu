...
double reduceScalar(memoryHolder<double> &cudaScalarAddress,
  ...
  while (N > 1) {
    ...
    callReduction1D(reduceIn->pointer(), reduceOut->pointer(), N, ngroups,
                    runningThreads);
    N = ngroups;
  }
  double toret;
...
std::vector<Vector> reduceNVectors(double *cudaNVectorAddress, unsigned N,
                                   unsigned nat, unsigned maxNumThreads) {
  ...
  cudaMemcpy(&toret[0][0], reduceOut, 3 * nat * sizeof(double),
...
std::vector<Vector> reduceNVectors(memoryHolder<double> &cudaNVectorAddress,
  ...
  while (N > 1) {
    ...
    callReductionND(reduceIn->pointer(), reduceOut->pointer(), N, ngroups,
                    runningThreads);
    N = ngroups.x;
  }
  std::vector<Vector> toret(nat);
