...
class TestInputBuilder(unittest.TestCase):
    ...
    def test_cv_special_access(self):
        self.check(
            ib.TORSION__("phi", ATOMS="5,7,9,15"), "phi: TORSION ATOMS=5,7,9,15\n"
        )

    def test_cv_space_separed_arguments(self):
        self.check(
            ib.TORSION("phi", ATOMS="5 7 9 15"), "phi: TORSION ATOMS={5 7 9 15}\n"
        )

    def test_cv_subkeyword(self):
        self.check(
            ib.COORDINATION(
                GROUPA="1-10", GROUPB="11-20", SWITCH="RATIONAL NN=6 R_0=1"
            ),
  ...
  """,
    ...
    def test_cv_flag_off(self):
        self.check(
            ib.DISTANCE("d", ATOMS="11 21", NOPBC=False), "d: DISTANCE ATOMS={11 21}\n"
        )

    def test_cv_args_integer_float(self):
        self.check(
            ib.METAD(
                ARG="phi psi", PACE=500, HEIGHT=1.2, SIGMA="0.35 0.35", FILE="HILLS"
            ),
    ...
    def test_cv_numbered_explicit(self):
        self.check(
            ib.MOVINGRESTRAINT(
                ARG="d1", KAPPA0=0, KAPPA1=10.0, AT0=20, AT1=20, STEP0=1, STEP1=10000
            ),
    ...
    def test_numbered(self):
        self.check(
            ib.MOVINGRESTRAINT(
                ARG="d1",
                KAPPA=ib.numbered([0, 10.0]),
                AT=ib.numbered([20, 20]),
                STEP=ib.numbered([1, 10000]),
            ),
            "MOVINGRESTRAINT ARG=d1 AT0=20 AT1=20 KAPPA0=0 KAPPA1=10.0 STEP0=1 STEP1=10000\n",
        )
    ...
    def test_numbered_set_tuple(self):
        self.check(
            ib.MOVINGRESTRAINT(
                ARG="d1",
                KAPPA=ib.numbered([100]),
                AT=ib.numbered({0: 0.0, 2: 10.0}),
                STEP=ib.numbered((0, 10, 20, 30)),
            ),
    ...
    def test_numpy_input(self):
        self.check(
            ib.RESTRAINT(
                ARG="d1", KAPPA=10, AT=ib.replicas(numpy.linspace(3.0, 5.0, 17))
            ),
    ...
    def test_replicas_array(self):
        self.check(
            ib.RESTRAINT(
                ARG="d1,d2", KAPPA=(10, 10), AT=ib.replicas(([0.0, 1.0], [10.0, 11.0]))
            ),
    ...
    @unittest.skipIf(
        not _HAS_MDANALYSIS, "Test runnable only if MDAnalysis is installed"
    )
