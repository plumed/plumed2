...
## Version 2.10.2 (in progress)
  ...
  - Fixed conversion of negative (and zero) float scalars from mixed-precision MD codes.
    ...
    The rounding that recovers a clean timestep is now requested with the new PUT flag
    ROUND_TO_BASE_TEN, and used for the timestep only, since its grid is coarser than the
    float itself and was discarding up to 4 bits of the energy.
