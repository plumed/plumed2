...
if [[ $1 = "prepare" ]]; then
    ...
    cat <<EOF
# sets up the tests with the CPU
$0 reset
#then prepares the plumed.dat with the USEGPU
$0 prepare
#then runs the GPU tests
EOF
    exit 0
fi
...
if [[ $action = "prepare" ]]; then
    ...
    inputLineScaledComponents[${#inputLineScaledComponents[@]}]="DISTANCE ATOMS1=1,2 ATOMS2=3,4 ATOMS3=5,6 SCALED_COMPONENTS  $USEGPU"
    ...
    inputLineScaledComponents[${#inputLineScaledComponents[@]}]="POSITION ATOM1=1 ATOM2=3 ATOM3=5 SCALED_COMPONENTS  $USEGPU"
