...
/*PLUMED*/
#include "gromacs/applied_forces/plumed/PlumedOutside.h"
/*ENDPLUMED*/
...
static void exchange_state(const gmx_multisim_t* ms, int b, t_state* state)
{
    ...
    exchange_rvecs(ms, b, state->x.rvec_array(), state->numAtoms());
    exchange_rvecs(ms, b, state->v.rvec_array(), state->numAtoms());
}
...
static real calc_delta(FILE* fplog, gmx_bool bPrint, struct gmx_repl_ex* re, int a, int b, int ap, int bp)
{
    ...
    /*PLUMED*/
    if (PLMD::plumedOutside().plumedHREX())
    {
                                      ...
                                      real                  time)
{
    ...
    if (plumed_test_exchange_pattern && PLMD::PlumedOutside().plumedHREX())
        gmx_fatal(FARGS, "hrex not compatible with ad hoc exchange patterns");
    /* END PLUMED */
    ...
    else
    {
        ...
        /* PLUMED */
        if (PLMD::PlumedOutside().active())
        {
            int partner = re->repl;
            PLMD::PlumedOutside().cmd("getExchangesFlag", &plumed_test_exchange_pattern);
            if (plumed_test_exchange_pattern > 0)
            {
                int* list;
                snew(list, re->nrepl);
                PLMD::PlumedOutside().cmd("setNumberOfReplicas", &(re->nrepl));
                PLMD::PlumedOutside().cmd("getExchangesList", list);
                for (i = 0; i < re->nrepl; i++)
                    re->ind[i] = list[i];
                sfree(list);
            }
            ...
            PLMD::PlumedOutside().cmd("GREX setPartner", &partner);
            PLMD::PlumedOutside().cmd("GREX calculate", nullptr);
            PLMD::PlumedOutside().cmd("GREX shareAllDeltaBias", nullptr);
        }
        /* END PLUMED */
        ...
        for (i = 1; i < re->nrepl; i++)
        {
            ...
            if (i % 2 == m)
            {
                ...
                if (PLMD::PlumedOutside().active())
                {
                    ...
                    char buf[300];
                    sprintf(buf, "GREX getDeltaBias %d", a);
                    PLMD::PlumedOutside().cmd(buf, &adb);
                    sprintf(buf, "GREX getDeltaBias %d", b);
                    PLMD::PlumedOutside().cmd(buf, &bdb);
                          ...
                          real                  time)
{
    ...
    /* PLUMED */
    if (PLMD::plumedOutside().active())
        PLMD::plumedOutside().cmd("GREX prepare", nullptr);
    /* END PLUMED */
