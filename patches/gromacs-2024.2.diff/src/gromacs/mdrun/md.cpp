...
void gmx::LegacySimulator::do_md()
{
    ...
    int  nstSignalComm               = nstglobalcomm;
    {
        ...
        simulationsShareState = useReplicaExchange || usingEnsembleRestraints || awhUsesMultiSim || (plumedswitch && ms_); // PLUMED hack, if we have multiple sim and plumed we usually want them to be in sync 
    ...
    if(plumedswitch){
      ...
      if(pversion>2){
        int res=1;
        if( (startingBehavior_ != StartingBehavior::NewSimulation) ) plumed_cmd(plumedmain,"setRestart",&res);
      }

      if(isMultiSim(ms_)) {
        if(MAIN(cr_)) plumed_cmd(plumedmain,"GREX setMPIIntercomm",&ms_->mainRanksComm_);
        if(PAR(cr_)){
          if(haveDDAtomOrdering(*cr_)) {
            plumed_cmd(plumedmain,"GREX setMPIIntracomm",&cr_->dd->mpi_comm_all);
          }else{
            plumed_cmd(plumedmain,"GREX setMPIIntracomm",&cr_->mpi_comm_mysim);
          }
        }
      ...
      if(PAR(cr_)){
        if(haveDDAtomOrdering(*cr_)) {
          plumed_cmd(plumedmain,"setMPIComm",&cr_->dd->mpi_comm_all);
        }
      }
      plumed_cmd(plumedmain,"setNatoms",topGlobal_.natoms);
      plumed_cmd(plumedmain,"setMDEngine","gromacs");
      plumed_cmd(plumedmain,"setLog",fpLog_);
    ...
    while (!bLastStep)
    {
        ...
        if (plumedswitch && bHREX) {
          ...
          int nlambda = enerd_->foreignLambdaTerms.numLambdas();
          gmx_enerdata_t hrex_enerd(enerd_->grpp.nener, nlambda == 0 ? nullptr: &inputRec_->fepvals->all_lambda);
          int repl  = -1;
          int nrepl = -1;
          if (MAIN(cr_)){
          ...
          if (haveDDAtomOrdering(*cr_)) {
            ...
            copy_state_serial(state_, stateGlobal_);
          }

          if(MAIN(cr_)){
            if(repl%2==step/replExParams_.exchangeInterval%2){
              if(repl-1>=0) exchange_state(ms_,repl-1,stateGlobal_);
            }else{
              if(repl+1<nrepl) exchange_state(ms_,repl+1,stateGlobal_);
            }
          }
          if (!haveDDAtomOrdering(*cr_)) {
            copy_state_serial(stateGlobal_, state_);
          }
          if (haveDDAtomOrdering(*cr_)) {
              dd_partition_system(fpLog_,
                                  mdLog_,
            ...
            || !mdGraph->useGraphThisStep())
        {
            ...
            else
            {
                ...
                if(plumedswitch){
                  ...
                  plumed_cmd(plumedmain,"setPositions",&state_->x[0][0]);
                  plumed_cmd(plumedmain,"setMasses",&md->massT[0]);
                  plumed_cmd(plumedmain,"setCharges",&md->chargeA[0]);
                  plumed_cmd(plumedmain,"setBox",&state_->box[0][0]);
            ...
            if(plumedswitch){
              if(plumedNeedsEnergy){
                ...
                plumed_cmd(plumedmain,"setEnergy",&enerd_->term[F_EPOT]);
              ...
              if(bHREX) plumed_cmd(plumedmain,"GREX cacheLocalUNow",&enerd_->term[F_EPOT]);
            }
            /* END PLUMED */
            }
