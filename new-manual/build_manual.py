            ...
            if "//+ENDPLUMEDOC" in line:
                ...
                for key in line.split():
                    ...
                    if key in syntax.keys() or key in syntax["cltools"].keys():
                        if len(actioname) > 0:
                            raise Exception(
                                "found more than one action name in input for " + key
                            )
                        actioname = key
                    else:
                        taglist = key + " "
        ...
        # get info from doi
            ...
            cit = (
                subprocess.check_output(
                    'curl -LH "Accept: text/bibliography; style=science" \'http://dx.doi.org/'
                    + doi
                    + "'",
                    shell=True,
                )
...
def printDataTable(f, titles, tabledata, tagdictionary={}, extraAttributes={}):
    if "Tags" in titles:
        if titles[2] != "Tags":
            raise Exception("found tag column in surprising location")
        ...
        f.write(
            "<button class=\"dropbtn\" onclick='clearTagSelection()'>Tags</button>\n"
        )
        f.write('<div class="dropdown-content">\n')
        for tag in mytags:
            f.write(
                '<a href="#" onclick=\'displayActionWithTags("'
                + tag
                + "\")' onmouseover='displayTagData(\""
                + tag
                + "\")' onmouseleave='displayTagData(\"no selection\")'>"
                + tag
                + "</a>\n"
            )
        f.write("</div>\n")
    ...
    for r in tabledata:
        if len(r) != len(titles):
            raise Exception(
                "mismatch between number of columns in tabledata and number of titles"
            )
        f.write("<tr>\n")
        for e in r:
...
def createSummaryPage(broken, nodocs, undocumented, noexamples, unexempled_keywords):
    with open("docs/summary.md", "w+") as f:
        f.write(
            "# Summary of manual coverage\n\n Data on this page was generated on "
...
"""
    clf.write(content)
    versions = sorted(
        glob.glob("../CHANGES/*.md"),
        key=lambda x: Version(x.split("/")[-1].split(".md")[0]),
    )
    for version in versions:
        shutil.copy(version, "docs/" + version.split("/")[-1])
        clf.write(
            "- Changes for [Version "
            + version.split("/")[-1].replace("v", "").replace(".md", "")
            + "]("
            + version.split("/")[-1]
            + ")\n"
        )
...
def getModuleType(module):
    if not Path("../../src/" + module + "/module.type").is_file():
        return "always"
    with open("../../src/" + module + "/module.type") as f:
        return f.read().strip()


def drawModuleNode(index, key, of):
    of.write(str(index) + '("' + key + '")\n')
    if getModuleType(key) == "always":
        of.write("style " + str(index) + " fill:blue\n")
    elif getModuleType(key) == "default-on":
        of.write("style " + str(index) + " fill:green\n")
    elif getModuleType(key) == "default-off":
        of.write("style " + str(index) + " fill:red\n")
    else:
        raise Exception("don't know how to draw node of type " + getModuleType(key))
...
"""
            ...
            for cyc in cycles:
                for i in range(len(cyc) - 1):
                    G.remove_edge(cyc[i], cyc[(i + 1) % len(cyc)])
            ...
            for edge in pG.edges():
                graphmat[edge[0], edge[1]] = 1
            for cyc in cycles:
                for i in range(len(cyc) - 1):
                    graphmat[cyc[i], cyc[(i + 1) % len(cyc)]] = 1

            drawn = np.zeros(k)
            for i in range(k):
                if backtranslate[i] == "core":
                    continue
                ...
                for j in range(k):
                    if (
                        np.sum(graphmat[:, i]) > 0
                        and np.all(graphmat[:, j] == graphmat[:, i])
                        and drawn[j] == 0
                    ):
                        group.add(j)
            ...
            for i in range(k):
                if drawn[i] == 0:
                    drawModuleNode(i, backtranslate[i], of)
                for j in range(k):
                    if graphmat[i, j] > 0:
...
def getModuleDictionary(modname):
    if os.path.exists("../../src/" + modname + "/module.yml"):
        with open("../../src/" + module + "/module.yml") as f:
...
def createModulePage(
    ...
    with open("module_" + modname + ".md", "w") as f:
        ...
        if os.path.exists("../../src/" + modname + "/module.md"):
            with open("../../src/" + modname + "/module.md") as iff:
                docs = iff.read()
            actions = set()
            ...
            if nf[0] > 0:
                broken_inputs.append(
                    [
                        '<a href="../module_' + modname + '">' + modname + "</a>",
                        str(nf[0]),
                    ]
                )
        foundaction = False
        for key, value in plumed_syntax.items():
            if (
                ...
                or key == "groups"
                or key == "cltools"
                or key != value["displayname"]
                or value["module"] != modname
            ):
                continue
            foundaction = True
            break
        ...
        if modname == "cltools":
            if foundaction:
                raise Exception(
                    "Found action in cltools module. There should be no actions in this module"
                )
            ...
            if foundaction:
                ...
                for key, value in plumed_syntax.items():
                    ...
                    tabledata.append(
                        [alink, str(value["description"]), str(plumedtags[key])]
                    )
                printDataTable(
                    f, ["Name", "Description", "Tags"], tabledata, tagdictionary
                )

            foundcltool = False
            for key, value in plumed_syntax["cltools"].items():
                if value["module"] != modname:
                    continue
                foundcltool = True
                break
...
def getKeywordDescription(docs):
    ...
    if "actionlink" in docs.keys() and docs["actionlink"] != "none":
        desc = (
            desc
            + ". Options for this keyword are explained in the documentation for ["
            + docs["actionlink"]
            + "]("
            + docs["actionlink"]
            + ".md)."
    ...
    if "pagelink" in docs.keys() and docs["pagelink"] != "none":
        desc = (
            desc
            + ". Further information about this flag can be found [here]("
            + docs["pagelink"]
            + ")."
        )
    return desc
...
def createCLToolPage(
    ...
    with open("docs/" + tool + ".md", "w") as f:
        ...
        if tool in plumeddocs.keys():
            ...
            nodocs.append(['<a href="../' + tool + '">' + tool + "</a>", "cltool"])
        f.write("\n## Syntax \n")
        ...
        if "dois" in value and len(value["dois"]) > 0:
            f.write("\n## References \n")
            f.write(
                "More information about how this cltool can be used is available in the following articles:\n"
            )
    ...
    with open(action + ".md", "w") as f:
        ...
        if nlessons > 0:
            f.write(
                ...
                + " tutorials](https://img.shields.io/badge/tutorials-"
                + str(nlessons)
                + "-green.svg)](https://www.plumed-tutorials.org/browse.html?action="
                + action
                + ")"
            )
        else:
        ...
        if neggs > 0:
            f.write(
                ...
                + " eggs](https://img.shields.io/badge/nest-"
                + str(neggs)
                + "-green.svg)](https://www.plumed-nest.org/browse.html?action="
                + action
                + ")"
            )
        else:
        ...
        if "output" in value["syntax"] and "value" in value["syntax"]["output"]:
            ...
            f.write(
                "| "
                + value["syntax"]["output"]["value"]["description"]
                + " | "
                + value["syntax"]["output"]["value"]["type"]
                + " |\n\n"
            )
        ...
        if "replacement" in value:
            ...
            f.write(
                "    This action has been deprecated and is no longer supported. Use ["
                + value["replacement"]
                + "]("
                + value["replacement"]
                + ".md) instead.\n\n"
        ...
        if hasatoms or hasargs:
            ...
            for key, docs in value["syntax"].items():
                if key == "output":
                    continue
                if docs["type"] == "atoms":
                    f.write("| " + key + " | atoms | " + docs["description"] + " |\n")
                elif "argtype" in docs.keys():
                    f.write(
                        "| "
                        + key
        ...
        if "output" in value["syntax"] and len(value["syntax"]["output"].keys()) > 1:
            ...
            if (
                "value" in value["syntax"]["output"]
                and len(value["syntax"]["output"]) == 1
            ):
                pass
            else:
                onlydefault = True
                ...
                if onlydefault:
                    f.write(
                        "This action calculates the [values](specifying_arguments.md) in the following table.  These [values](specifying_arguments.md) can be referenced elsewhere in the input by using this Action's label followed by a dot and the name of the [value](specifying_arguments.md) required from the list below.\n\n"
                    )
...
def actionPage(
    key, plumed_syntax, nest_map, school_map, plumedtags, version, plumeddocs
):
    ...
    with cd("docs"):
        broken_input, undocumented_keyword, noexample, nodoc, unexempled_keywords = (
            createActionPage(
                version,
                key,
                value,
...
if (
    ...
    actions = set()
    pages = glob.glob("*.md")
    run_action_page = functools.partial(
        generateGeneralPages, plumed=PLUMED, version=version
    )
    if sys_platform == "wasi" or sys_platform == "ios":
        # multiprocessing is not avaiable on WASI or iOS
        # https://docs.python.org/3/library/multiprocessing.html
    ...
    run_action_page = functools.partial(
        actionPage,
        plumed_syntax=plumed_syntax,
        nest_map=nest_map,
        school_map=school_map,
        plumedtags=plumedtags,
        version=version,
    ...
    # append
    broken_inputs += [
        x["broken_input"] for x in actiontable if x["broken_input"] is not None
    ]
    undocumented_keywords = [
        x["undocumented_keyword"]
        for x in actiontable
        if x["undocumented_keyword"] is not None
    ]
    ...
    unexempled_keywords = [
        x["unexempled_keywords"]
        for x in actiontable
        if x["unexempled_keywords"] is not None
    ]
    ...
    for module, value in modules.items():
        ...
        if not os.path.exists("../src/" + module + "/module.md") or not os.path.exists(
            "../src/" + module + "/module.yml"
        ):
            nodocs.append(
